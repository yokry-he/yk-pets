...
export class PluginRegistry {
    ...
    register(manifest, module) {
        ...
        if (!report.compatible)
            throw new PluginCompatibilityError(report);
        ...
        for (const capability of manifest.capabilities?.provides ?? []) {
            const existing = this.#capabilities.get(capability.id);
            if (existing)
                existing.owners.push(manifest.id);
            else
                this.#capabilities.set(capability.id, { owners: [manifest.id], exclusive: capability.exclusive !== false });
        }
        return record;
    }
    unregister(id) {
    ...
    activationOrder(ids = [...this.#plugins.keys()]) {
        ...
        const visit = (id, chain) => {
            if (visited.has(id))
                return;
            if (visiting.has(id))
                throw new Error(`Plugin dependency cycle: ${[...chain, id].join(' -> ')}`);
            const record = this.#plugins.get(id);
            if (!record)
            ...
            for (const [dependencyId, range] of Object.entries(record.manifest.dependencies ?? {})) {
                ...
                if (!dependency)
                    throw new Error(`Missing required plugin dependency: ${dependencyId}`);
                if (!satisfiesVersion(dependency.manifest.version, range))
                    throw new Error(`Incompatible plugin dependency ${dependencyId}: ${dependency.manifest.version} does not satisfy ${range}`);
            ...
            for (const requirement of record.manifest.capabilities?.requires ?? []) {
                ...
                if (!providerId)
                    throw new Error(`Missing required capability: ${requirement.id}`);
                if (providerId !== id) {
...
export function validatePluginManifest(manifest) {
    const issues = [];
    if (manifest.schema !== 'yk-pets.plugin/v1')
        issues.push(error('schema', 'Unsupported plugin schema', 'schema'));
    if (!/^[a-z][a-z0-9-]*(?:\.[a-z][a-z0-9-]*)+$/.test(manifest.id))
        issues.push(error('id', 'Plugin id must be reverse-domain-like lowercase segments', 'id'));
    if (!manifest.name?.trim())
        issues.push(error('name', 'Plugin name is required', 'name'));
    if (!parseVersion(manifest.version))
        issues.push(error('version', 'Plugin version must be semantic version', 'version'));
    if (!manifest.engines?.ykPets)
        issues.push(error('engine', 'engines.ykPets is required', 'engines.ykPets'));
    if (!Object.values(manifest.entrypoints ?? {}).some(value => Array.isArray(value) ? value.length : Boolean(value)))
        issues.push(error('entrypoint', 'At least one entrypoint is required', 'entrypoints'));
    if (manifest.integrity && !/^[a-f0-9]{64}$/i.test(manifest.integrity.digest))
        issues.push(error('integrity', 'SHA-256 digest must contain 64 hexadecimal characters', 'integrity.digest'));
    const provides = manifest.capabilities?.provides ?? [];
    if (new Set(provides.map(item => item.id)).size !== provides.length)
        issues.push(error('capability-duplicate', 'Provided capability ids must be unique', 'capabilities.provides'));
    const dependencies = manifest.dependencies ?? {};
    if (manifest.id in dependencies)
        issues.push(error('dependency-self', 'Plugin cannot depend on itself', 'dependencies'));
    for (const [id, range] of Object.entries({ ...manifest.dependencies, ...manifest.optionalDependencies })) {
        if (!id || !range || !isValidRange(range))
            issues.push(error('dependency-range', `Invalid dependency range for ${id}`, `dependencies.${id}`));
    }
    return issues;
}
export function checkPluginCompatibility(manifest, environment) {
    ...
    if (manifest.engines?.petCore) {
        if (!environment.petCoreVersion)
            issues.push(error('engine-pet-core-missing', 'pet-core runtime is unavailable', 'engines.petCore'));
        else if (!satisfiesVersion(environment.petCoreVersion, manifest.engines.petCore))
            issues.push(error('engine-pet-core', `pet-core ${environment.petCoreVersion} does not satisfy ${manifest.engines.petCore}`, 'engines.petCore'));
    }
    if (manifest.engines?.api) {
        if (!environment.apiVersion)
            issues.push(error('engine-api-missing', 'Plugin API runtime is unavailable', 'engines.api'));
        else if (!satisfiesVersion(environment.apiVersion, manifest.engines.api))
            issues.push(error('engine-api', `Plugin API ${environment.apiVersion} does not satisfy ${manifest.engines.api}`, 'engines.api'));
    }
    for (const tool of manifest.permissions?.tools ?? [])
        if (!matchesPermission(environment.allowedTools, tool))
            issues.push(error('permission-tool', `Tool permission is not allowed: ${tool}`, 'permissions.tools'));
    for (const scope of manifest.permissions?.scopes ?? [])
        if (!matchesPermission(environment.allowedScopes, scope))
            issues.push(error('permission-scope', `Scope permission is not allowed: ${scope}`, 'permissions.scopes'));
    for (const origin of manifest.permissions?.origins ?? [])
        if (!matchesPermission(environment.allowedOrigins, origin))
            issues.push(error('permission-origin', `Origin permission is not allowed: ${origin}`, 'permissions.origins'));
...
export function satisfiesVersion(version, range) {
    if (!parseVersion(version) || !isValidRange(range))
        return false;
...
export function isValidRange(range) {
    ...
    return range.split('||').every(group => group.trim().split(/\s+/).filter(Boolean).every(token => {
        if (token === '*' || /^\d+(?:\.x|\.\*)?(?:\.x|\.\*)?$/.test(token))
            return true;
        if (/^[~^]?\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(token))
            return true;
        if (/^(?:>=|<=|>|<|=)\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(token))
            return true;
        return false;
...
function satisfiesToken(version, token) {
    if (token === '*')
        return true;
    if (/^\d+$/.test(token))
        return parseVersion(version).major === Number(token);
    const wildcard = token.match(/^(\d+)\.(x|\*|\d+)(?:\.(x|\*|\d+))?$/);
    if (wildcard && (wildcard[2] === 'x' || wildcard[2] === '*' || wildcard[3] === 'x' || wildcard[3] === '*')) {
        const parsed = parseVersion(version);
        if (parsed.major !== Number(wildcard[1]))
            return false;
        if (wildcard[2] !== 'x' && wildcard[2] !== '*' && parsed.minor !== Number(wildcard[2]))
            return false;
        if (wildcard[3] && wildcard[3] !== 'x' && wildcard[3] !== '*' && parsed.patch !== Number(wildcard[3]))
            return false;
        return true;
    }
...
function matchesPermission(allowed, requested) {
    if (!allowed)
        return false;
    return allowed.some(pattern => permissionPatternCovers(pattern, requested));
}
function permissionPatternCovers(allowed, requested) {
