...
export function createStableSelector(node, maxDepth = 6) {
    const segments = [];
    let current = node;
    for (let depth = 0; current && depth < maxDepth; depth += 1) {
        ...
        const testAttribute = ['data-testid', 'data-test', 'data-qa', 'data-cy']
            .map(name => [name, getAttribute(current, name)])
        ...
        if (nth.count > 1)
            segment += `:nth-of-type(${nth.index})`;
        segments.unshift(segment);
        current = current.parentElement;
    }
    return segments.join(' > ') || '*';
}
export function createDomDescriptor(node, options = {}) {
...
export function inspectVueOwnership(element) {
    ...
    if (vue3 && typeof vue3 === 'object')
        return inspectVue3(vue3);
    const vue2 = record.__vue__;
    if (vue2 && typeof vue2 === 'object')
        return inspectVue2(vue2);
    return null;
}
export function parseVueInspectorMetadata(value) {
...
export function decodeSourceMapMappings(map) {
        ...
        for (const encoded of segments) {
            ...
            if (values.length >= 4) {
                ...
                if (values.length === 5) {
                    ...
                    if (previousName < 0)
                        throw new Error('Source Map name index became negative');
...
export function flattenSourceMapMappings(map) {
    validateSourceMap(map);
    if (!('sections' in map))
        return decodeSourceMapMappings(map);
    const output = [];
...
export function findOriginalMapping(mappings, generatedLine, generatedColumn) {
    if (!Number.isSafeInteger(generatedLine) || generatedLine < 0 || !Number.isSafeInteger(generatedColumn) || generatedColumn < 0)
        return null;
    let candidate = null;
...
export function normalizeSourcePath(value) {
    ...
    if (drive)
        output = `${drive[1].toUpperCase()}:/${output.slice(drive[0].length)}`;
    return output;
}
function inspectVue3(instance) {
...
function collectVue3Parents(value) {
    const output = [];
...
function collectVue2Parents(value) {
    const output = [];
    let current = objectRecord(value);
    const seen = new Set();
    while (current && output.length < 12 && !seen.has(current)) {
...
function findInspectorLocation(attributes) {
    for (const attribute of ['data-v-inspector', 'data-vue-source', 'data-source-location']) {
        ...
        if (parsed)
            return { attribute, ...parsed };
    }
    return null;
}
function readUnknownAttributes(record) {
    ...
    return readAttributes(record, 50);
}
function readAttributes(node, max) {
    const output = {};
    const source = node.attributes;
    if (source && !Array.isArray(source) && typeof source === 'object' && !('length' in source)) {
        for (const [name, value] of Object.entries(source).slice(0, max))
            if (typeof value === 'string')
                output[name] = sanitizeAttribute(value);
        return output;
    }
    if (source && typeof source === 'object' && 'length' in source && typeof source.length === 'number') {
        ...
        for (let index = 0; index < Math.min(list.length, max); index += 1) {
            ...
            if (item && typeof item.name === 'string' && typeof item.value === 'string')
                output[item.name] = sanitizeAttribute(item.value);
        }
    }
    return output;
}
...
function validateSourceMap(map) {
    if (!map || typeof map !== 'object' || map.version !== 3)
        throw new Error('Only Source Map v3 is supported');
    if ('sections' in map) {
        if (!Array.isArray(map.sections))
            throw new Error('Indexed source map sections are required');
        let previousLine = -1;
...
function validateRegularSourceMap(map) {
    if (map.version !== 3 || !Array.isArray(map.sources) || typeof map.mappings !== 'string')
        throw new Error('Invalid Source Map v3 payload');
...
function decodeVlqSegment(segment) {
    ...
    for (const character of segment) {
        ...
        if (digit === undefined)
            throw new Error(`Invalid Source Map base64 character: ${character}`);
    ...
    if (shift !== 0)
        throw new Error(`Incomplete Source Map VLQ segment: ${segment}`);
    return output;
}
...
function sourceMapLeafForPosition(map, line, column) {
    if (!('sections' in map))
        return { map, line, column };
    let selected = null;
    for (const section of map.sections) {
        if (section.offset.line > line || (section.offset.line === line && section.offset.column > column))
            break;
        selected = section;
    }
    if (!selected)
...
function resolveSourcePath(source, sourceRoot, generatedUrl) {
    const root = sourceRoot ?? '';
    try {
        if (/^[a-z][a-z0-9+.-]*:/i.test(source))
            return normalizeSourcePath(source);
        if (/^[a-z][a-z0-9+.-]*:/i.test(root))
            return normalizeSourcePath(new URL(source, root.endsWith('/') ? root : `${root}/`).toString());
...
function findByPath(maps, generatedUrl) {
