...
export const REPOSITORY_GATE_SCHEMA = 'yk-pets.repository-gate/v1';
...
export function evaluateCommitGate(input) {
    ...
    check(checks, 'head-revision', input.snapshot.headRevision === input.expectedBaseRevision, input.snapshot.headRevision === input.expectedBaseRevision
        ? 'The worktree HEAD matches the approved base revision'
        : 'The worktree HEAD does not match the approved base revision');
    check(checks, 'attached-branch', !input.snapshot.detached && input.snapshot.branch !== null, !input.snapshot.detached && input.snapshot.branch ? `Changes are on branch ${input.snapshot.branch}` : 'Detached HEAD is not allowed');
    ...
    check(checks, 'protected-branch', !protectedBranch, protectedBranch ? `Direct commits to protected branch ${branch} are forbidden` : 'The target branch is not protected');
    ...
    check(checks, 'branch-prefix', allowedBranch, allowedBranch ? 'The branch uses an approved prefix' : `Branch must start with one of: ${policy.allowedBranchPrefixes.join(', ')}`);
    ...
    check(checks, 'changes-present', actualPaths.length > 0, actualPaths.length > 0 ? `${actualPaths.length} changed path(s) are present` : 'No repository changes are present');
    ...
    check(checks, 'approved-path-set', exact, exact ? 'Changed paths match the approved scope' : describePathMismatch(extra, missing), [...new Set([...extra, ...missing])]);
    const protectedPaths = actualPaths.filter(path => isProtectedPath(path, policy.protectedPathPrefixes));
    check(checks, 'protected-paths', protectedPaths.length === 0, protectedPaths.length === 0 ? 'No protected repository paths are changed' : 'Protected repository paths cannot be committed', protectedPaths);
    ...
    check(checks, 'allowed-path-prefixes', outsideAllowed.length === 0, outsideAllowed.length === 0 ? 'All changed paths are inside allowed prefixes' : 'Some changed paths are outside allowed prefixes', outsideAllowed);
    ...
    check(checks, 'staged-state', !policy.requireAllChangesStaged || unstaged.length === 0, !policy.requireAllChangesStaged || unstaged.length === 0 ? 'All approved changes are staged' : 'Unstaged changes remain in the worktree', [...new Set(unstaged)].sort());
    const untracked = input.snapshot.changes.filter(change => change.status === 'untracked').map(change => normalizeProjectPath(change.path));
    check(checks, 'untracked-files', policy.allowUntracked || untracked.length === 0, policy.allowUntracked || untracked.length === 0 ? 'Untracked-file policy is satisfied' : 'Untracked files are not allowed', untracked);
    check(checks, 'file-budget', actualPaths.length <= policy.maxFiles, actualPaths.length <= policy.maxFiles ? `Changed file count is within ${policy.maxFiles}` : `Changed file count exceeds ${policy.maxFiles}`);
    check(checks, 'byte-budget', changedBytes <= policy.maxChangedBytes, changedBytes <= policy.maxChangedBytes ? `Changed byte estimate is within ${policy.maxChangedBytes}` : `Changed byte estimate exceeds ${policy.maxChangedBytes}`);
    ...
    check(checks, 'required-validations', validationFailures.length === 0, validationFailures.length === 0 ? 'All required validations passed' : `Required validations did not pass: ${validationFailures.join(', ')}`);
    ...
    check(checks, 'secret-scan', blockingSecrets.length === 0, blockingSecrets.length === 0 ? 'No blocking secret findings were reported' : 'Blocking secret findings prevent commit', blockingSecrets.map(finding => finding.path));
    return {
...
export function assertCommitGatePassed(decision) {
    if (decision.schema !== REPOSITORY_GATE_SCHEMA)
        throw new Error('Unsupported repository gate decision');
    if (!decision.passed) {
...
export async function computeCommitGateDigest(decision) {
    return sha256Hex(stableStringify(decision));
}
export function createChangeBranchName(planId, summary, options = {}) {
...
export function validateBranchName(branch) {
    if (typeof branch !== 'string' || branch.length === 0 || branch.length > 200)
        throw new Error('Branch name length is invalid');
    if (branch.startsWith('-') || branch.startsWith('/') || branch.endsWith('/') || branch.endsWith('.') || branch.includes('..') || branch.includes('@{'))
        throw new Error('Branch name is invalid');
    if (/\s|[~^:?*\[\\\x00-\x1f\x7f]/.test(branch) || branch.split('/').some(part => part.length === 0 || part.startsWith('.') || part.endsWith('.lock')))
        throw new Error('Branch name is invalid');
    return branch;
}
function validateSnapshot(snapshot) {
    ...
    if (snapshot.branch !== null)
        validateBranchName(snapshot.branch);
    if (typeof snapshot.detached !== 'boolean' || typeof snapshot.clean !== 'boolean')
        throw new Error('Repository snapshot flags are invalid');
    if (typeof snapshot.headRevision !== 'string' || !SHA_PATTERN.test(snapshot.headRevision))
        throw new Error('Repository head revision is invalid');
    if (!Array.isArray(snapshot.changes))
        throw new Error('Repository changes are invalid');
    for (const change of snapshot.changes)
        validateChange(change);
}
function validateChange(change) {
    if (!isRecord(change) || !['added', 'modified', 'deleted', 'renamed', 'copied', 'untracked'].includes(change.status))
        throw new Error('Repository change is invalid');
    normalizeProjectPath(change.path);
    if (change.oldPath !== undefined)
        normalizeProjectPath(change.oldPath);
    if ((change.status === 'renamed' || change.status === 'copied') && !change.oldPath)
        throw new Error(`${change.status} change requires oldPath`);
    if (typeof change.staged !== 'boolean')
        throw new Error('Repository change staged flag is invalid');
    if (change.bytesAdded !== undefined)
        nonNegative(change.bytesAdded, 'bytesAdded');
    if (change.bytesDeleted !== undefined)
        nonNegative(change.bytesDeleted, 'bytesDeleted');
}
function normalizePolicy(policy = {}) {
    return {
        ...
        failOnSecretSeverities: [...new Set(policy.failOnSecretSeverities ?? ['high', 'critical'])],
        forbiddenCommitSubjectPatterns: policy.forbiddenCommitSubjectPatterns ?? DEFAULT_FORBIDDEN_SUBJECTS,
    };
}
function validateCommitSubject(subject, policy) {
    if (typeof subject !== 'string')
        return 'Commit subject must be a string';
    const trimmed = subject.trim();
    if (trimmed !== subject || trimmed.length === 0)
        return 'Commit subject must be non-empty and have no surrounding whitespace';
    if (new TextEncoder().encode(subject).byteLength > policy.maxCommitSubjectLength)
        return `Commit subject exceeds ${policy.maxCommitSubjectLength} bytes`;
    if (subject.endsWith('.'))
        return 'Commit subject must not end with a period';
    for (const pattern of policy.forbiddenCommitSubjectPatterns) {
        pattern.lastIndex = 0;
        if (pattern.test(subject))
            return `Commit subject matches forbidden pattern ${String(pattern)}`;
    }
    return null;
}
function normalizeValidations(input) {
    const ids = new Set();
    return input.map(item => {
        if (!isRecord(item))
            throw new Error('Validation evidence is invalid');
        const id = boundedText(item.id, 'validation id', 100);
        if (ids.has(id))
            throw new Error(`Duplicate validation evidence: ${id}`);
        ids.add(id);
        if (!['passed', 'failed', 'skipped'].includes(item.status))
            throw new Error(`Validation ${id} status is invalid`);
        boundedText(item.summary, `validation ${id} summary`, 1_000);
        if (!Number.isFinite(item.startedAt) || !Number.isFinite(item.completedAt) || item.startedAt <= 0 || item.completedAt < item.startedAt)
            throw new Error(`Validation ${id} timestamps are invalid`);
        if (item.command !== undefined)
            boundedText(item.command, `validation ${id} command`, 500);
        if (item.artifactDigest !== undefined && !/^[a-f0-9]{64}$/.test(item.artifactDigest))
            throw new Error(`Validation ${id} artifact digest is invalid`);
        return { ...item };
    });
}
function normalizeSecretFindings(input) {
    const ids = new Set();
    return input.map(finding => {
        if (!isRecord(finding))
            throw new Error('Secret finding is invalid');
        const id = boundedText(finding.id, 'secret finding id', 100);
        if (ids.has(id))
            throw new Error(`Duplicate secret finding: ${id}`);
        ids.add(id);
        const path = normalizeProjectPath(finding.path);
        if (!['low', 'medium', 'high', 'critical'].includes(finding.severity))
            throw new Error(`Secret finding ${id} severity is invalid`);
        ...
        if (finding.line !== undefined && (!Number.isInteger(finding.line) || finding.line <= 0))
            throw new Error(`Secret finding ${id} line is invalid`);
        if (finding.redactedPreview !== undefined)
            boundedText(finding.redactedPreview, `secret finding ${id} preview`, 200);
        return { ...finding, path };
    });
}
function changedPaths(changes) {
    ...
    for (const change of changes) {
        ...
        if (change.oldPath)
            paths.add(normalizeProjectPath(change.oldPath));
    }
    return [...paths].sort();
}
function normalizeUniquePaths(paths, label) {
...
function describePathMismatch(extra, missing) {
    const parts = [];
    if (extra.length)
        parts.push(`unapproved: ${extra.join(', ')}`);
    if (missing.length)
...
function normalizePrefix(prefix) {
    if (typeof prefix !== 'string' || prefix.length === 0)
        throw new Error('Path prefix is invalid');
...
function validateBranchPrefix(prefix) {
    if (typeof prefix !== 'string' || !prefix.endsWith('/') || prefix.length > 80)
        throw new Error('Branch prefix must end with /');
    validateBranchName(`${prefix}x`);
    return prefix;
}
function check(checks, id, passed, message, paths) {
