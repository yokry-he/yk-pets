 ...
 */
const DEFAULT_STATE = { behavior: 'idle', emotion: 'neutral', score: 80 };
export class CloudFox2DScene {
    state = { ...DEFAULT_STATE };
    elapsed = 0;
    update(deltaSeconds, state = this.state) {
        this.elapsed += Math.max(0, Math.min(0.1, deltaSeconds));
        this.state = { ...state, pointer: state.pointer ? { ...state.pointer } : undefined };
    }
    render(context, width, height, devicePixelRatio = 1) {
    ...
    #drawCloud(ctx, t, sleeping) {
        ...
        for (const [x, y, r] of [[-44, 3, 25], [-18, -5, 30], [15, -7, 33], [46, 4, 25]]) {
            ctx.beginPath();
            ctx.arc(x, y + Math.sin(t * 1.4 + x) * 1.5, r, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.restore();
    }
    #drawTail(ctx, t, excited, sleeping) {
        ctx.save();
        ...
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.bezierCurveTo(38, 4, 54, -32, 72, -55);
        ctx.stroke();
        ctx.strokeStyle = '#eef1ff';
        ctx.lineWidth = 12;
        ctx.beginPath();
        ctx.moveTo(56, -36);
        ctx.quadraticCurveTo(66, -46, 72, -55);
        ctx.stroke();
        ctx.shadowBlur = excited ? 18 : 8;
        ctx.shadowColor = '#b4a8ff';
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(73, -56, excited ? 7 : 5, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
    }
    #drawBody(ctx, breathing, sleeping) {
        ctx.save();
        ...
        ctx.beginPath();
        ctx.ellipse(0, 0, 48, 54, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#dfe4ff';
        ctx.beginPath();
    ...
    #drawHead(ctx, t, thinking, sleeping) {
        ctx.save();
        ...
        ctx.beginPath();
        ctx.ellipse(0, 0, 45, 38, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (sleeping) {
            ctx.beginPath();
            ctx.moveTo(-23, 0);
            ctx.quadraticCurveTo(-15, 7, -7, 0);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(7, 0);
            ctx.quadraticCurveTo(15, 7, 23, 0);
            ctx.stroke();
        }
        else {
            ctx.fillStyle = '#252b55';
            ctx.beginPath();
            ctx.ellipse(-15, 1, 5, thinking ? 8 : 6, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(15, 1, 5, thinking ? 8 : 6, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#ffffff';
            ctx.beginPath();
            ctx.arc(-13.5, -1, 1.8, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(16.5, -1, 1.8, 0, Math.PI * 2);
            ctx.fill();
        }
    ...
    #drawPaws(ctx, t, waving, sleeping) {
        ...
        for (const side of [-1, 1]) {
            ...
            ctx.beginPath();
            ctx.ellipse(0, 14, 12, sleeping ? 17 : 25, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#edf0ff';
            ctx.beginPath();
            ctx.ellipse(0, sleeping ? 23 : 34, 10, 9, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
        }
    }
    #drawEnergy(ctx, t, active) {
        ctx.save();
        ...
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(0, -10, 5 * pulse, 0, Math.PI * 2);
        ctx.fill();
        if (active) {
            ctx.strokeStyle = '#b7adff';
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.arc(0, -10, 12 + Math.sin(t * 4) * 2, 0, Math.PI * 2);
            ctx.stroke();
        }
        ctx.restore();
    }
...
export class Canvas2DPetRenderer {
    ...
    mount(target) {
        if (typeof document === 'undefined' && !this.options.canvasFactory)
            throw new Error('Canvas2DPetRenderer requires a DOM canvas factory');
    ...
    setVisible(visible) {
        ...
        if (visible && this.options.autoStart)
            this.start();
        else if (!visible)
            this.stop();
    }
    ...
    resize(width, height, devicePixelRatio = this.#dpr) {
    ...
    restore(snapshot) {
        ...
        if (typeof snapshot.payload?.elapsed === 'number')
            this.scene.elapsed = snapshot.payload.elapsed;
        this.drawFrame(0);
    }
    dispose() {
        this.stop();
...
export function createCanvas2DRendererFactory(options = {}) {
    return {
        kind: '2d',
        create: () => new Canvas2DPetRenderer(options),
    };
}
export function defineYkPets2DElement(tagName = 'yk-pets-2d') {
    if (typeof customElements === 'undefined' || typeof HTMLElement === 'undefined')
        return null;
    const existing = customElements.get(tagName);
    if (existing)
        return existing;
    class YkPets2DElementImpl extends HTMLElement {
        ...
        disconnectedCallback() { this.renderer.dispose(); }
    }
