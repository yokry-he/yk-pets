...
const files = {
  ...
  playgroundHome: readFileSync(new URL('../apps/playground/app/pages/index.vue', import.meta.url), 'utf8'),
  playgroundStore: readFileSync(new URL('../apps/playground/app/stores/pet.ts', import.meta.url), 'utf8'),
  playgroundChat: readFileSync(new URL('../apps/playground/server/api/pet-command.post.ts', import.meta.url), 'utf8'),
...
const expectations = [
  ...
  ['native playground brand', files.playgroundHome.includes('<span class="brand-mark">YK</span>') && files.playgroundHome.includes('YK-PETS / ZEPH CLOUD FOX EXPERIMENT')],
  ['native playground pet function', files.playgroundHome.includes('function petZeph()') && !files.playgroundHome.includes('function petNOVA()')],
  ['playground runtime pet id', files.playgroundStore.includes("id: 'zeph'") && files.playgroundStore.includes("speciesId: 'cloud-fox'")],
  ['playground localized identity state', files.playgroundStore.includes("'zh-CN': '云灵'") && files.playgroundStore.includes("en: 'Cloud Fox'")],
  ['playground storage migration', files.playgroundStore.includes("const STORAGE_KEY = 'yk-pets:playground:pet-state:v2'") && files.playgroundStore.includes("const LEGACY_STORAGE_KEY = 'nuxt-ai-pet-state-v1'")],
  ['playground chat identity', files.playgroundChat.includes('云灵，英文名 Zeph，物种是一只云狐') && files.playgroundChat.includes('不要把产品品牌当成你的名字')],
