...
const checks = [
  ...
  ['复杂模式在唯一 Canvas 内实际替换 renderer，复杂分支消费预览变换且简单模式保持现有 renderer', (canvasTemplate.match(/<TresCanvas\b/g) || []).length === 1 && /<TresGroup\b[^>]*v-if="showComplexRenderer"[^>]*:position="vec3\(previewPosition\)"[^>]*:rotation="vec3\(complexPreviewRotation\)"[^>]*:scale="vec3\(\[previewScale,\s*previewScale,\s*previewScale\]\)"/.test(canvasTemplate) && /<ComplexBipedPetRenderer\b[^>]*@compilation="onComplexCompilation"/.test(canvasTemplate) && /<ProceduralPet\b[^>]*v-else/.test(canvasTemplate) && canvasScript.includes("modelMode: 'simple'")],
