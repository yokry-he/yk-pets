 ...
 * 锁定扩展与 Studio 共用模型的经典参数、单一头身表面、完整部件栈、相机灯光和无损视觉质量。
 * Locks classic parameters, sole head/body surfaces, the complete part stack, cameras, lighting, and lossless quality shared by extension and Studio.
 */
...
const checks = [
  ...
  ['complete sole torso head intent and continuous tail remain', headIntent.includes('ExtensionCloudFoxHead') && body.includes('<ExtensionCloudFoxBodyShape') && body.includes('frontPawDesign') && bodyShape.includes('sole production torso surface') && head.includes('<ExtensionCloudFoxHeadShape') && headShape.includes('independently selectable head shell') && tail.includes('TresTubeGeometry') && tail.includes('tipGlow.enabled')],
  ['body and head profiles are independent but share one coordinate system', shapeProfiles.includes('CloudFoxBodyProfile') && shapeProfiles.includes('CloudFoxHeadProfile') && shapeProfiles.includes('getCloudFoxBodyProfile') && shapeProfiles.includes('getCloudFoxHeadProfile')],
