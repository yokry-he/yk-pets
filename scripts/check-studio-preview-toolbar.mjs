...
const checks = [
  ...
  ['动作时间仅在高级编辑中使用可选控制栏扩展', toolbar.includes('showTime') && motion.includes(':show-time="editor.authoringMode === \'advanced\'"')],
  ...
  ['三个工坊暂不绑定滚轮缩放', workspaces.every(source => !source.includes('@wheel') && !source.includes('wheelPreview'))],
