...
const checks = [
  ...
  ['one animated head owns one independent shell and one FaceRoot', head.includes('<ExtensionCloudFoxHeadShape') && head.includes('<ExtensionCloudFoxFaceCustomization') && headShape.includes('getCloudFoxHeadProfile') && headShape.includes('capsuleGeometry') && !face.includes('ref="head"')],
