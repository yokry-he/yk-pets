...
const checks = [
  ...
  ['Studio 一次性把 v2/v1 资产迁移到 v3 并清空旧动作', workspace.includes("STUDIO_ASSET_STORAGE_KEY = 'yk-pets:studio:assets:v3'") && workspace.includes("STUDIO_ASSET_V2_STORAGE_KEY = 'yk-pets:studio:assets:v2'") && workspace.includes("STUDIO_ASSET_V1_STORAGE_KEY = 'yk-pets:studio:assets:v1'") && workspace.includes('normalizeStudioAssetHydration') && workspace.includes('resetLegacyMotions') && store.includes('normalizeStudioAssetHydration') && store.includes('localStorage.removeItem(STUDIO_ASSET_V2_STORAGE_KEY)') && store.includes('localStorage.removeItem(STUDIO_ASSET_V1_STORAGE_KEY)') && store.includes('authoringAppearanceId') && !store.includes('appearanceId: input.appearanceId')],
