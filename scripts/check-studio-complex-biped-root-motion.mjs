...
expect(/'weapon-trail':\s*24/.test(weaponVfxSource) && /'impact-sparks':\s*32/.test(weaponVfxSource) && /'impact-ring':\s*8/.test(weaponVfxSource), '持械特效池容量必须固定为 24/32/8 / Weapon-VFX pool capacities must remain fixed at 24/32/8')
expect(weaponVfxSource.includes('InstancedMesh') && weaponVfxSource.includes('RingGeometry') && weaponVfxSource.includes('PlaneGeometry'), '持械特效必须复用轨迹 Mesh、火花 InstancedMesh 与冲击环 Geometry / Weapon VFX must reuse trail meshes, spark instances, and ring geometry')
expect(/WEAPON_TRAIL_SAMPLE_INTERVAL_MS\s*=\s*40/.test(motionAdaptationSource) && motionAdaptationSource.includes('sampleBipedPetWeaponVfxSignals'), '武器轨迹信号必须使用固定 40ms 确定采样 / Weapon-trail signals must use deterministic fixed 40ms sampling')
