 ...
 * 校验默认、Studio 和导入配方都进入同一个完整可配置 Cloud Fox 组件，并保持显式肚皮、鼻嘴、前后爪与独立颜色迁移。
 * Verifies that default, Studio, and imported recipes enter one complete customizable Cloud Fox component while preserving belly, nose/mouth, front/hind paws, and independent color migration.
 */
...
const bellyPaths = ['width','height','offsetX','offsetY','rotation','softness']
const checks = [
  ['belly migration is explicit and ellipse-first', customization.includes('PetBellyCustomizationRecipe') && customization.includes('legacyBellyShape') && customization.includes("mode!=='custom'") && customization.includes("return'ellipse'") && customization.includes('normalizeBelly')],
  ['Studio exposes every belly shape and shared numeric controls', editor.includes('PET_BELLY_SHAPES') && bellyIds.every(id => customization.includes(`id:'${id}'`) || customization.includes(`id: '${id}'`)) && editor.includes('恢复椭圆默认') && editor.includes('StudioNumericControl') && bellyPaths.every(key => controls.includes(`'customization.belly.${key}'`) && editor.includes(`'customization.belly.${key}'`))],
  ...
  ['one customization normalizer handles every recipe source', configured.includes('normalizeCustomizableAppearance') && configured.includes('createExtensionClassicAppearance') && customization.includes('normalizeNose') && customization.includes('normalizeHindPaw')],
  ...
  ['canonical renderer consumes complete appearance channels', body.includes('frontPawDesign') && body.includes('hindPawDesign') && body.includes('symbols.chest') && head.includes('earDesign') && face.includes('nose.value.scaleX') && face.includes('appearance.parts.mouth') && tail.includes('tailDesign') && core.includes('ExtensionCloudFoxOrbit')],
  ['Studio exposes nose and hind-paw imported fields', mouthEditor.includes("'customization.nose.scaleZ'") && mouthEditor.includes("'customization.nose.rotation'") && hindEditor.includes('hindPawDesign.pawScaleZ') && hindEditor.includes('hindPawDesign.toeOutwardAngle')],
