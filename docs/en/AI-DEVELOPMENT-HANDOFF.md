...
- The task-2 `pet-core` suite contained 207 passing tests. Coverage includes direct timeline components/transitions; equivalent low-overlap and low-adjacent topology; true-threshold touchdown; exact adjacency; `.001/.1/1-ULP` positive gaps; centered and off-center overlap-internal grounded valleys; true-peak stability for micro windows before, at, and after the main peak and for multiple micro windows; both sides of the `.25/.4/.45` intensity thresholds; true peak, single-touchdown, and frame-subdivision consistency for 2/3/4/5/6 regularly offset windows; Bernstein local restriction and global weight normalization; the fixed-seed 40-case offset budget ceiling; inclusive canonical horizontal-support endpoints across once, loop seams, and ping-pong turnarounds; exact-seam segment sides and timestamp ties; conservative unrepresentable-iteration fallback; bidirectional once/loop/ping-pong anchors; huge iterations; and frame subdivision. The versioned probe still runs 10,000 seeded stateful cases, 42 forward/reverse ULP touchdowns, the canonical 64-window authorization chain, and six high/low exact-adjacency subdivision cases. Its new `compositePeakStability` field records the four before/center/after/multiple-micro-window true-peak cases, while `offsetOverlapBudget` records 40 fixed-seed 2–6-window cases with `exhausted=0`, a maximum of `179/512` work units, and a locked ceiling of `192`. The 63 tail peaks remain at or below about `2.9999986e-12`, under the `4e-12` world-ground threshold; the 64-window shared analysis still contains 129 structural boundaries, 64 support components, and 502 of 512 work units with `exhausted=false`. Stable categories remain `1707 solved / 8272 clamped / 10 reset / 11 blocked`, 1,169 loop seams, 344 applied landings, and 3,978 brake signals; all 42 ULP probes pass. Each 2,000-frame one/64-window continuous observation still reports 200 authorization-carrying frames and 100 real impulse consumptions. Timings remain observational with no machine-dependent threshold. Task 3 now supplies deterministic signal derivation, but the Three runtime consumer and VFX pool remain absent; `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.

## 31. Deterministic biped-pet motion-VFX signals

- `@yk-pets/pet-core` now exports the framework-neutral `deriveBipedPetMotionVfxSignals`. It only combines solved Root Motion `motionIntensity`, `landingImpulse`, and `brakeIntensity` with explicit action `vfxTags`; it never infers effects from render state, wall-clock time, or hidden history. Missing authorization, `reset/blocked`, rewind, and repeated-time samples return a fresh frozen empty array.
- The first four effects use strict greater-than thresholds: `landing-ring=.25`, `landing-dust=.4`, `speed-trail=.55`, and `brake-sparks=.45`; equality stays silent. `landingImpulse>0` is the one-shot authority for an actual touchdown. The current solver classifies the same frame as `grounded` as it crosses the ground threshold, so landing effects accept `grounded|landing` and reject `takeoff|airborne`. Braking sparks require `grounded`, while speed trails may sustain through valid moving phases. Landing rings, dust, and braking sparks are `burst` signals identified by ``${clipHash}:${kind}:${Math.round(requestedTimeMs)}``; speed trails are `sustain` signals identified by ``${clipHash}:${kind}:active``. Signals use full Unicode code-point order and deduplicate repeated tags.
- The public boundary rejects negative/non-finite request time, blank, control-character, over-256-code-point, or non-string Clip hashes and safely contains malformed arrays, invalid state/phase, non-finite intensities, and throwing Proxy getters without mutation or escaped exceptions. Every returned array and signal is independently frozen. Current lifetimes are `480ms` for landing ring/dust, `160ms` for speed trails, and `320ms` for braking sparks, all below the future Three pool's `1200ms` cap.
- TDD first observed all nine initial tests fail because the public API was absent, then used separate RED cycles for negative time and the real ballistic-touchdown integration boundary. The complete `pet-core` suite now contains 217 tests covering tag authorization, strict threshold sides, burst deduplication, stable sustain identity, repeated/rewound/reset/blocked time, a real solver grounded touchdown, phase gating, invalid Clip hashes, Unicode identity, non-finite signals, malformed/Proxy inputs, non-mutation, deep freezing, and non-shared outputs.
