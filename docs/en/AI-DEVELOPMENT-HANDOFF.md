...
- Direct IK retains the historical contract: single support does not translate pelvis Y and the approximately `0.014` unreachable `100→320ms` walk case remains clamped. A full search of the allowed `0.025×height` pelvis X/Z disk after applying container travel found a best world residual of about `0.00825`, so `<1e-3` is not reachable without stretching bones or adding vertical reach. Only the integrated motion controller therefore enables an opt-in single-support pelvis-Y strategy bounded by `min(0.08, characterHeight×0.025)`. IK remains the sole pelvis-Y owner, reports clamping when that budget is exhausted, restores bind Y for airborne/no-support/zero-weight/reset/dispose, and never rewrites foot/ankle local positions or segment lengths.
- The outer order is fixed to `restoreBindPose → FK → Root Motion → Balance → updateMatrixWorld → IK`. Automated coverage includes frame-rate convergence, target-debt chasing, pause authorization, stop/rewind/Clip replacement, nonzero pitched bind transforms and world-axis yaw, character-scale stride, coarse/split touchdown, double/single/airborne balance, default-off and integrated height-scaled pelvis Y, blocked Root Motion with continuing FK/IK, frozen reports/diagnostics, residual handoff and gap clearing, local-position/segment-length preservation, and disposal boundaries. This completes only the Three controller layer: renderer wiring and the VFX pool remain pending, so `bipedPetRootMotionThreeRuntimeComplete=true` while `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.
