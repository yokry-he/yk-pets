...
- `@yk-pets/pet-core` now provides the pure numeric `sampleBipedPetRootMotion` contract. Absolute-time `cumulativeLocal/World` and `cumulativeTurnRadians` remain deterministic target values, while caller-owned `previousAppliedLocal/previousAppliedTurnRadians` produce budgeted `appliedLocal/World`, `appliedTurnRadians`, deltas, and velocities that match the state a future runtime controller may actually write. A missing applied state or reset initializes applied to target without emitting velocity or transient events; clamped frames retain target debt for later frames.
...
- Ballistic windows are weight-normalized into a composite airborne signal. Phase direction uses the analytic smoothstep-arc derivative. A common positive scale prevents reciprocal overflow, then a relative summation-error bound derived from absolute slope contributions, `Number.EPSILON`, and the bounded active-window count classifies numerical zero. This preserves a main window under an active threshold-weight micro-window while rejecting roundoff left by mathematically cancelling symmetric windows. Every candidate boundary is retained only when a bounded probe sees composite height above the shared `1e-12` grounded epsilon on the physical before-side and grounded height on the after-side. Exact adjacency is explicitly continuous, while the after-side probe is capped before the nearest distinct boundary so any positive gap remains observable; overlapping, duplicate, adjacent, and epsilon-adjacent tail windows therefore cannot manufacture or hide a touchdown. `landingImpulse` is a bounded dimensionless heuristic based on jump intent, action weight, and the ending contributors' share of all effective ballistic weight; it is not presented as physical endpoint velocity. Loop and ping-pong sampling aggregate one signal even when a valid short-duration interval crosses several touchdowns.
- Root Motion normalization, compilation caches, time resolution, and sampling share `normalizeMotionDurationMs`. Normalized definitions are recursively frozen and bound to their canonical duration in a WeakMap, allowing a trusted zero-copy sampling path while unknown or duration-mismatched public input remains defensively copied and validated. The 100,000-sample local microbenchmark recorded about `100.73ms` for one window and `235.74ms` for 64 windows.
- The full `pet-core` suite contains 161 passing tests, including mutable legacy-cache invalidation, non-finite/throwing residual Y, whole-frame support-boundary crossings, negligible/shared-end tails, overlap/exact-adjacency/narrow-positive-gap touchdown probes, representable-interior ULP-scale windows in both ping-pong directions, active threshold-weight micro-window phase direction, and symmetric-slope cancellation. A separate 10,000-input seeded stateful probe recorded `4583 solved / 5406 clamped / 11 reset / 10 blocked`, plus 2,055 loop seams, 2,053 real landing signals, and 2,922 brake signals in about `32.48ms`. The Three runtime consumer and task-3 VFX implementation are still absent; `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.
