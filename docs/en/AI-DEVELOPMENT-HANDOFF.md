...
- The authoritative specification is `docs/zh-CN/双足萌宠混合RootMotion与运动特效设计.md`. Only the design is complete; Root Motion, motion VFX, production quadruped/mech Profiles, high-detail topology, and cross-browser GPU acceptance remain incomplete.
- The executable plan is `docs/zh-CN/双足萌宠混合RootMotion与运动特效实施计划.md`, split into eight TDD batches. Every feature-source commit must update the AI state and one handoff context before it is pushed. Only the plan is complete; production Root Motion and VFX implementation has not started.
