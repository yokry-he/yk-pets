...
- First frame, stop, rewind, Clip replacement, solver reset, runtime rebuild, and disposal clear old applied state, debt, and authorization. Pause keeps the pending authorization without motion or signals. Coarse and boundary-split canonical touchdown sequences consume one equal impulse each; Three never reconstructs, endpoint-snaps, or rescales the authorization. The motion frame also exposes a deeply frozen Bone-free IK report and finite local horizontal residual handoff with Y forced to zero. `residualByLimb` is explicitly the true world-horizontal `hypot(anchor.x-current.x, anchor.z-current.z)` magnitude for each limb; a 3D distance may not leak pure-Y error into horizontal feedback. In-place motion, travel gaps, and airborne phases clear residual ownership without drift.
- `ComplexBipedBalanceController` exclusively owns pelvis X/Z and chest Quaternion. Its horizontal vector stays within `characterHeight×0.025`, chest tilt stays within `0.12rad`, and standalone continuity removes only its own prior tilt while preserving a newly written external FK pose. Double support centers the pelvis, single support biases toward the supporting side according to actual movement intensity, and airborne/reset/dispose restores owned bind state.
- IK `apply()` accepts an optional readonly `rootMotionPhase` frame context from the full chain. Actual applied `takeoff/airborne` takes precedence over possibly stale nonzero authored contact weights: IK restores pelvis bind Y, clears anchors, and returns an empty support report. Standalone callers that omit the context retain the historical contact semantics. Direct IK also retains its three alternating passes, keeps single-support pelvis Y unchanged, and leaves the approximately `0.014` unreachable `100→320ms` walk case clamped. A full search of the allowed `0.025×height` pelvis X/Z disk after applying container travel found a best world residual of about `0.00825`, so `<1e-3` is not reachable without stretching bones or adding vertical reach. Only the integrated motion controller therefore enables a single-support pelvis-Y strategy bounded by `min(0.08, characterHeight×0.025)` and five alternating convergence passes under the same per-bone cumulative angular budget. The fixed world-residual probe improves from `0.000999317248683272` to about `0.00031089757658926703`, with a `≤0.00075` gate. IK remains the sole pelvis-Y owner, reports clamping when that budget is exhausted, and never rewrites foot/ankle local positions or segment lengths.
- The outer order is fixed to `restoreBindPose → FK → Root Motion → Balance → updateMatrixWorld → IK`. Automated coverage includes frame-rate convergence, target-debt chasing, pause authorization, stop/rewind/Clip replacement, nonzero pitched bind transforms and world-axis yaw, character-scale stride, coarse/split touchdown, stale-contact release during real takeoff/airborne phases, pure-Y residual isolation, double/single/airborne balance, default-off and integrated height-scaled pelvis Y, unchanged three/five-pass angular budgets, the `≤7.5e-4` integrated residual margin, blocked Root Motion with continuing FK/IK, frozen reports/diagnostics, residual handoff and gap clearing, local-position/segment-length preservation, and disposal boundaries. This completes only the Three controller layer: renderer wiring and the VFX pool remain pending, so `bipedPetRootMotionThreeRuntimeComplete=true` while `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.
