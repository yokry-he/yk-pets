...
- The movement-intensity reference was corrected from an unusable `4` to the exported `0.4` body-heights-per-second constant while still reading only actual applied horizontal speed. Natural-playback tests keep `actionWeight=1` throughout and carry applied position, turn, and landing authorization frame by frame: the 20ms walk probe yields 46 trail frames; the 10ms sprint probe yields 582 trail frames and 36 brake-spark frames, including `7300ms`; the 40ms jump probe yields one real applied touchdown with `sqrt(.28)≈.529` and exactly one landing-ring plus one landing-dust burst in that frame. Weight jumps, fabricated target debt, and tag-specific intensity overrides are not used.
...
## 33. Three Root Motion, balance, and IK coordination

- The new `ComplexBipedRootMotionController` snapshots the container bind position/Quaternion and owns only the current Clip's `previousAppliedWorld`, `previousAppliedTurnRadians`, and sampler-issued readonly landing authorization. It writes `bindPosition + appliedWorld` and `worldYaw * bindQuaternion` absolutely, never accumulates `delta*`, never writes `cumulative*`, and reuses temporary Three vectors on the frame path. Character height comes only from the computed geometry bounding box; an invalid box blocks Root Motion independently while FK/IK continues.
- First frame, stop, rewind, Clip replacement, solver reset, runtime rebuild, and disposal clear old applied state, debt, and authorization. An exact repeat of Clip, requested time, and normalized weight freezes the complete display pose and pending residual: only Root maintains its time token and authorization, VFX stays empty, `consumedFootResidual` is zero, and resumed progression performs the real consumption. Coarse and boundary-split canonical touchdown sequences consume one equal impulse each; Three never reconstructs, endpoint-snaps, or rescales authorization. `residualByLimb` is the scalar world-horizontal `hypot(anchor.x-current.x, anchor.z-current.z)` strain heuristic, not a direction vector. The outer controller scans the readonly record directly, applies an `×8` gain bounded by `characterHeight×0.025` along reverse local movement, and reuses one frozen zero for every early return. Root first previews with zero residual and only resamples from the identical previous state when the preview is finite `solved/clamped + grounded`; the second pure sample must also remain finite grounded or the controller commits the preview instead. This prevents both stale strain at takeoff and horizontal strain stealing the shared XYZ budget from an already-proven touchdown, so authorization, impulse, and VFX cannot be delayed into landing. Y stays zero; in-place motion, travel gaps, and non-support phases clear ownership without drift.
- `ComplexBipedBalanceController` owns pelvis X/Z and only its added chest tilt. Its horizontal vector stays within `characterHeight×0.025`, chest tilt stays within `0.12rad`, and standalone continuity removes only its own prior tilt while preserving the current authored FK pose. The hot path reuses Quaternion/Euler instances and scans contacts once without `filter`. Double support centers the pelvis, single support biases toward the supporting side, and `takeoff/airborne/landing` clears balance compensation without overwriting the frame's FK chest pose.
- IK `apply()` accepts an optional readonly `rootMotionPhase` frame context. Capture, pelvis correction, solving, and reporting share one effective-support rule: finite positive weight and finite positive confidence. Actual applied `takeoff/airborne/landing` overrides stale authored contacts: IK restores pelvis bind Y, clears anchors, and returns an empty report; only a real grounded touchdown recaptures. Standalone callers retain historical semantics. Direct IK keeps three passes and the approximately `0.014` clamped walk residual. Only the integrated controller enables single-support pelvis Y bounded by `min(0.08, characterHeight×0.025)` and five passes under the same per-bone angular budget; its fixed probe remains about `0.00031089757658926703` under the `≤0.00075` gate.
- A WeakMap token permits exactly one Root Motion writer per runtime; owner disposal releases it and disposed runtimes reject creation. Outer construction failure preserves the initialization error while best-effort cleaning IK, Balance, and Root; safe total string conversion covers Symbol-valued messages and throwing getters, so diagnostics cannot interrupt cleanup. Root disposal deletes its token and seals itself in `finally`, with a separate failure sentinel so even `throw undefined` is rethrown unchanged. Outer disposal attempts IK, Balance, Root, runtime-state inspection, bind restoration, and matrix update in order; a failed state check is aggregated and bind/matrix are still attempted. It clears references and seals itself before throwing one Chinese aggregate, and a second disposal is idempotent. Normal frames retain `restoreBindPose → FK → Root Motion → Balance → updateMatrixWorld → IK`; duplicate pause frames use the freeze path. Coverage adds the exact once travel/ballistic touchdown budget boundary, full-pose pause, stale-contact release throughout landing plus grounded recapture, invalid support states, exclusive-owner and exceptional construction/disposal lifecycles, shared-zero identity, and long loop plus genuinely compiled and sampled ping-pong assets without reversal or overshoot. That batch completed only the Three controller layer; section 34 now adds the independent VFX pool, but production renderer wiring still remains pending, so `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.

## 34. Bounded same-scene motion-VFX pool

- The new `createComplexBipedMotionVfxController` returns a plain Three `Group` that task 7 can mount as a sibling of the complex-character primitive in the existing scene. This batch creates no Canvas, Skeleton, parallel scene, or animation loop and does not modify the production renderer. `apply()` consumes only the sampled `requestedTimeMs`, the character container's final position in the shared parent, and facing; it reads no `Date.now()` and schedules no `requestAnimationFrame()`. A burst captures its parent-space origin at trigger time, while a sustain signal updates the existing slot with the current character position.
- Each effect kind owns one fixed Geometry and Material. Landing ring, landing dust, speed trail, and braking sparks use physical capacities `8/24/8/24`, exactly `64` total; dust and sparks are real `InstancedMesh` pools. A burst creates at most `16` instances, trails retain at most `8`, and lifetime is capped at `1200ms`. All 64 slot-state objects are preallocated and reused. Equal-expiry instances rotate fairly by monotonic activation order instead of repeatedly overwriting slot zero. The recent burst ledger is a fixed 256-entry ring buffer without `shift()`; a bounded scan of the 64 active slots separately prevents a still-active ID from replaying after ledger eviction.
- The input boundary ignores non-finite/non-positive strength, non-finite time or position, invalid kind/mode, blank or oversized IDs, and non-positive lifetime; excessive lifetime is clamped and an unrepresentable deadline is ignored. Frame time, facing, and all coordinates are each read once into plain finite values, preventing a Proxy from injecting `NaN` after validation. Duplicate bursts do not allocate again, sustain IDs update in place, and expiry, rewind, or `reset()` clears active instances. Explicit reset also clears deduplication state while retaining pools and GPU resources. Facing follows Root Motion local `+Z`, using world forward `(sin(yaw), 0, cos(yaw))`; yaw `0/π÷2` maps to `+Z/+X`.
- Effect classes initialize independently. Default callbacks are lazy-cached, so each kind/resource class creates at most one provisional even when the callback is invoked repeatedly and then throws. A later failed kind never disposes a shared resource already committed to an earlier usable pool. Successful pools own Geometry/Material through identity sets, so each shared GPU object is released once. The controller also tracks every created child and detaches it from whichever parent an `attachObject` hook selected.
- `dispose()` first seals synchronous re-entry with `disposing`, so Geometry/Material `dispose` event listeners cannot recursively release resources twice, and re-entrant `apply/reset` calls are rejected while cleanup is in progress. It then independently attempts active-slot cleanup, every unique Geometry and Material, every owned child, the controller parent, and child clearing. Thrown `undefined`, a direct `Symbol`, Symbol-valued messages, and throwing message getters are safely formatted into one Chinese aggregate without stopping later cleanup. References are always cleared and the controller sealed; later calls remain idempotent and never release a GPU resource twice.
- TDD first observed `ERR_MODULE_NOT_FOUND` for the absent module. Coverage now exercises a real Group/InstancedMesh, true expired-ID replay, sustain updates, burst capture, `+Z` facing at yaw `0/π÷2`, fair equal-expiry eviction, active-ID ledger pressure, frame TOCTOU input, 200 dust bursts, the `64/16/8/1200` limits, partial attachment and provisional-factory failures, shared-resource later-kind failure, no wall clock or RAF, reset-with-retained-pools, unique-resource disposal, arbitrary thrown values, a real synchronous dispose-event re-entry, and arbitrary-parent detachment. Quality review reproduced and closed the 90-degree facing error, slot-zero overwrite, active-ID replay, Proxy-injected `NaN`, four-time shared disposal, 18 leaked external children, and two-time release under synchronous dispose-event re-entry; RED evidence is recorded in machine state. Machine state marks `bipedPetMotionVfxPoolComplete=true` and `biped-pet-motion-vfx-pool` complete. Production renderer mounting, full Root Motion/VFX delivery, and cross-browser GPU acceptance remain pending, so `bipedPetRootMotionComplete` and `bipedPetMotionVfxComplete` stay false.

## 35. Root Motion/VFX renderer and beginner settings

- `ComplexBipedPetRenderer` now creates exactly one `ComplexBipedMotionController` and one `ComplexBipedMotionVfxController` per character runtime. The VFX `Group` and character object are sibling primitives in the existing `TresCanvas`. After applying motion, the renderer copies the shared-parent-local `runtime.object.position` and rotates a reused local `+Z` vector by the final Quaternion before deriving facing with `atan2(x,z)`. It does not call `getWorldPosition`, create a Canvas/RAF/Skeleton, or allocate frame coordinate containers repeatedly.
- The Store now exposes monotonic `playbackRequestedTimeMs` for the complex runtime while editor UI, prop events, and timeline retain resolved `playheadTimeMs`. Loop seams and ping-pong reverse halves no longer look like rewinds; pause freezes requested time and resume continues from it, while open/replace, explicit scrub, and stop realign both clocks. Clip replacement, absent motion, blocked output, stopped weight, and genuine rewind explicitly reset motion and VFX. Motion/time watchers run through one re-entry-safe synchronization boundary whose operations return explicit success: missing clip/runtime paths cannot clear diagnostics, and recovery requires the matching failure domain/asset or a successful recompile. Stale clips detach before fallible reset/compile; failures best-effort reset, reset failures clear refs plus the runtime key, reverse-dispose, and permit same-recipe rebuild. Only bounded Chinese blocked diagnostics reach the parent. Runtime replacement, unmount, and partial construction failure retain VFX → motion → runtime cleanup. The existing `Complex v-if` / `ProceduralPet v-else` branch remains mutually exclusive.
- `StudioRootMotionSettings` is wired into the Basic motion-property tab and exposes only in-place/actual travel, automatic VFX, estimated distance/turn/jump, and recommendation restoration. Complex mode explains automatic body-size/contact/speed correction; Simple mode explains compatible metadata persistence. Movement choices use `aria-pressed`, automatic VFX uses `role=switch` plus `aria-checked`, and focus remains visible. The component keeps its inline-size container query below 360px and also makes movement modes and summaries one column at the motion workspace's 780px narrow-viewport breakpoint. The viewport rule is necessary because the mobile property panel expands close to full width, so its container remains wider than 360px at 760×900. Body/controls use 11–12px and supporting copy stays at least 10px.
- The Store now provides `updateRootMotionSettings` and `restoreRootMotionRecommendations`. One action replaces only `yk-pets/biped-motion/v1.rootMotion` in the current draft, preserves contacts, events, `sourceMotionId`, unknown namespace fields, and other extensions, and creates one undo entry. Mode and auto-VFX changes do not overwrite each other; disabling VFX only clears `vfxTags`. Restoration first uses the opened baseline's authored Root Motion and scales its windows by current/baseline duration; only a missing baseline definition permits an explicit versioned `sourceMotionId` to select built-in metadata, whose windows likewise scale to current duration. It never infers provenance from Chinese or English names and otherwise falls back to safe `.42` travel. `copyBuiltInMotion` writes the provenance field.
- The static gate now inspects Vue SFC template element/directive AST plus TypeScript `CallExpression/PropertyAccess` AST, so comments or string literals containing fake VFX calls/primitives cannot pass. TDD RED evidence covers the initial missing lifecycle/API, VFX mode isolation, unscaled jump windows, missing explicit rewind reset, requested `2500ms` collapsing to resolved `100ms`, absent watcher exception boundaries, bilingual-name custom motion being replaced by built-in travel, comment/string-only gate false positives, the previous 7px copy, empty frame synchronization incorrectly clearing a failure, a real copied jump retaining `[720,1824]` after `2400→1200ms`, and the 708px settings container at a 760px viewport failing to match the original 360px single-column condition. The final case first failed because no viewport rule could apply at 760px, then passed with the workspace-aligned 780px media query; a fresh real-browser recheck remains part of Task 8. At the Task 7 checkpoint, Task 8's delivery gate had not run, so the two phase-complete flags were still false; section 36 and machine state supersede that historical checkpoint.

## 36. Root Motion and motion-VFX phase delivery

- The first seven tasks now form one complete in-site path: versioned motion extensions declare intent, the framework-neutral sampler solves actual displacement from body height, contacts, and authored windows, the Three motion controller writes the character container absolutely in `restore-bind-pose → fk → root-motion → balance → update-matrix-world → ik` order, and the renderer forwards deterministic signals from the same requested time to the same-scene VFX pool. Root Motion exclusively owns the container position/Quaternion; FK, Balance, and IK retain their established local ownership, with no second writer accumulating the same world transform.
- Actual travel is explicit and currently limited to walk at `.42` body heights, sprint-stop at `2.4` body heights with a `6300→8300ms` brake window, and jump/landing with a `.28` body-height ballistic peak. Idle, wave, straight punch, and older assets remain `in-place`. The complex runtime consumes monotonic `playbackRequestedTimeMs`, while the timeline and prop events keep the resolved playhead, so loop seams and ping-pong reversal are no longer mistaken for rewinds.
- The current VFX set is `landing-ring`, `landing-dust`, `speed-trail`, and `brake-sparks`. Fixed capacities are `8/24/8/24`, exactly `64` slots; each burst is capped at `16`, trails at `8`, and lifetime at `1200ms`, with dust and sparks backed by `InstancedMesh`. The controller reads no wall clock and registers no RAF; rewind/reset/dispose clears active instances without rebuilding GPU resources on the hot path.
- `.ai/project-state.json` now marks Root Motion, motion VFX, both production-runtime wiring paths, and the phase delivery complete, and advances `nextPhase` to `biped-pet-advanced-choreography-warping-and-cross-browser-acceptance`. Production quadruped/mech Profiles, high-detail topology, and cross-browser GPU manual acceptance remain false; automated coverage of the low-detail biped runtime must not close those boundaries.
- `.ai/visual-cases.json` adds `biped-pet-root-motion-motion-vfx` with real Chromium evidence at both viewports. At 1440×900 the complex model stayed ready with one Canvas and no overflow; walk crossed two cycles, pause froze, stop reset, rewind did not crash; sprint showed continuous travel and braking posture, while a runtime probe confirmed 304 combined `speed-trail/brake-sparks` signals—the screenshot particles were subtle and are not claimed as clearly visible final pixels; jump visibly took off and landed; Simple/Complex remained exclusive, with zero console errors or hydration mismatches. After the fix at 760×900, the 708px settings panel computed one-column movement/summary tracks at 674px/684px, with no overflow, one Canvas, working jump playback controls and mode switching, and zero console/hydration errors; desktop was rechecked without regression. `latestCompletedBatch.browserCoverage` now records exactly both Chromium viewports. Safari/Firefox, other GPU/WebGL drivers, high-detail topology, and final pixels remain explicitly pending.
- The phase automation gate is fixed to root `typecheck`, root `test`, the Playground production build, documentation and AI-handoff gates, `git diff --check`, plus a simulated Pull Request history event using base `agent/yk-pets-rebrand-v0610` and the current HEAD. Results must come from fresh command output: the structural AI gate is not PR-history evidence, and automated checks cannot replace pending browser evidence.
- The simulated history gate reproduced four early Root Motion numerical-fix commits that updated handoff context but omitted `.ai/project-state.json`. This delivery adds no wildcard bypass: gate and machine state lock the exact SHAs `ad6ffeb7a5797cfb4df70b7b53e6b21b17d1c77e`, `b1975536849713b131220b3530521cf8b0f423ef`, `2ef7f4055667cd13635343e13570039abeba35d8`, and `b9b75560cc592417ce58af38efbe0b43f0c793cb`; the only permitted missing type is `project-state`, and the exception applies only when pushed closure `a52c2381da4b0ec8b474dcfb32d5eb32b1d16f89`—which updates both machine state and bilingual handoff context—is in the current history. The event base must also equal `agent/yk-pets-rebrand-v0610` exactly.
- On 2026-07-29, after the narrow-layout fix and both viewport rechecks, the full suite was rerun fresh: root `typecheck`, root `test`, the Playground production build, documentation, structural AI, diff, and the real simulated PR-history event all exited 0; `pet-core` was 220/220 and Local Agent 2/2. The build retained the known Nuxt sourcemap, VueUse `PURE` annotation-position, and over-500-kB chunk warnings; these warnings are neither hidden as “clean output” nor misreported as build failures.

## 37. Nebula Staff advanced motion-adaptation phase delivery

- A freshly copied “Nebula Staff Combo” now needs no GLB import, manual rigging, skin weighting, or particle setup. The in-site runtime resolves five explicit staff Rig points and compiles seven performance phases, five scaled Warp windows, three secondary-hand constraints, and five weapon-effect cues for the active biped proportions. The 12-second motion now contains 17 semantic tracks and 239 keyframes, including a vertical right-arm strike track that gives the impact point real downward velocity.
- Weapon impact remains trajectory-gated. A cue first uses a real downward ground crossing. When low-detail procedural geometry cannot guarantee that its endpoint crosses the ground, an optional phase-relative `triggerProgress` compiles to `triggerTimeMs`; a signal is emitted only when adjacent frames cross that authored instant and the actual sampled impact point is still moving downward faster than the threshold. First frame, repeated pause frames, rewind, and upward motion remain silent, so this is not an unconditional timed particle event.
- The built-in impact cue uses phase progress `.45`, resolving to `9660ms`. A real `40ms` integration probe over `9000→10100ms` runs the production motion, production staff Rig, and production weapon controller and observes both `impact-sparks` and `impact-ring`. The core regression also locks the one-shot fallback when an endpoint remains above ground but follows the required downward trajectory; `pet-core` is now `241/241`.
- Chromium acceptance on 2026-07-29 covered `450/2100/3800/6100/8500/9700/11200ms` at 1440×900. The seven phases, primary grip, three secondary-grip stages, support/air/recovery, spin and sweep trails were visible, and the `9700ms` strike clearly showed a cyan-white impact ring plus yellow sparks. Play, pause, stop, `9700→2100ms` rewind, Simple/Complex exclusivity, one Canvas, zero horizontal overflow, and a fresh page with zero console errors or hydration mismatches all passed.
- At 760×900 there was no horizontal overflow; the toolbar wrapped, the beginner summary and technical details became one column, playback and mode controls remained usable, the floating action did not block primary operations, and the page kept one Canvas. Evidence is recorded in `.ai/visual-cases.json` as `biped-pet-nebula-staff-adaptation`.
- This delivery closes only the vertical Nebula Staff sample and its reusable biped weapon-adaptation runtime. It does not close the full dance, boxing, gymnastics, or sprint library; human, quadruped, or mech Profiles; high-detail topology; Safari/Firefox and other GPU/WebGL paths; or final pixel/performance acceptance. The next phase is `biped-pet-motion-library-and-cross-browser-acceptance`.
- The first real Pull Request history event precisely found two already-pushed fixes that had not updated the AI package in the same commit: `8858386192bdc14511a931076984f5fba6dae5e2` (live preview-time positioning) and `db48fbcdca9190752e8a82728c8da555eef97417` (two-hand grip trajectory). Both actually lack `project-state` and `handoff-context`. There is no wildcard bypass: migration applies only to those full SHAs with the exact missing types and only while pushed phase closure `0e9fb37cd909cf6b94becc140c9f663e857f77d1`, which updates machine state and bilingual handoff context, is in current history. A wrong base, unknown SHA, extra missing type, or future feature commit still fails.
