...
- The full `pet-core` suite now contains 201 passing tests, adding direct component/transition coverage, request-endpoint independence, equivalent low-overlap/low-adjacent topology, true-threshold touchdown, exact-adjacency merging, `.001/.1/1-ULP` positive-gap splitting, centered and off-center overlap-internal grounded-valley splitting, resolver-canonical local endpoint inclusion, exact-seam segment-side preservation and timestamp-tie ordering, conservative handling of unrepresentable iteration anchors, once/loop/ping-pong anchors, huge-iteration bounds, and frame-subdivision invariance. The versioned probe still runs 10,000 seeded stateful cases, 42 forward/reverse ULP touchdowns, and the canonical 64-window authorization chain, and now adds six high/low exact-adjacency subdivision cases across once, loop, and ping-pong. The 63 tail peaks remain at or below about `2.9999986e-12`, under the `4e-12` world-ground threshold. The latest shared analysis contains 129 structural boundaries (requested endpoints removed), 64 support components, and 502 of 512 work units with `exhausted=false`. Stable categories remain `1707 solved / 8272 clamped / 10 reset / 11 blocked`, 1,169 loop seams, 344 applied landings, and 3,978 brake signals; all 42 ULP probes pass. Each 2,000-frame one/64-window continuous observation still reports 200 authorization-carrying frames and 100 real impulse consumptions. Timings remain observational with no machine-dependent threshold. The Three runtime consumer and task-3 VFX implementation are still absent; `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.
