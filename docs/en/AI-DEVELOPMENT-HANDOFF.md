...
- First frame, stop, rewind, Clip replacement, solver reset, runtime rebuild, and disposal clear old applied state, debt, and authorization. An exact repeat of Clip, requested time, and normalized weight freezes the complete display pose and pending residual: only Root maintains its time token and authorization, VFX stays empty, `consumedFootResidual` is zero, and resumed progression performs the real consumption. Coarse and boundary-split canonical touchdown sequences consume one equal impulse each; Three never reconstructs, endpoint-snaps, or rescales authorization. `residualByLimb` is the scalar world-horizontal `hypot(anchor.x-current.x, anchor.z-current.z)` strain heuristic, not a direction vector. The outer controller applies an `×8` gain bounded by `characterHeight×0.025` along reverse local movement. Root first previews with zero residual and only resamples from the identical previous state when the preview is finite `solved/clamped + grounded`, preventing a combined travel/ballistic takeoff from consuming stale strain. Y stays zero; in-place motion, travel gaps, and non-support phases clear ownership without drift.
...
- A WeakMap token permits exactly one Root Motion writer per runtime; owner disposal releases it, disposed runtimes reject creation, and an outer construction failure cleans up an already-acquired token. Normal frames retain `restoreBindPose → FK → Root Motion → Balance → updateMatrixWorld → IK`; duplicate pause frames use the freeze path. Coverage adds full-pose pause, travel+ballistic zero-residual phase preview, stale-contact release throughout landing plus grounded recapture, invalid support states, exclusive-owner lifecycle, constructor cleanup, and long loop/ping-pong strain sequences without reversal or overshoot. This completes only the Three controller layer: renderer wiring and the VFX pool remain pending, so `bipedPetRootMotionThreeRuntimeComplete=true` while `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.
