...
- Applied phase is derived from actual applied world height and the actual vertical delta: ground-threshold height is `grounded`, rising is `takeoff`, falling is `landing`, and stationary positive height is `airborne`. Every legal ballistic window end/start is retained as a forward/reverse touchdown candidate, with contributor weights aggregated once at equal boundaries; fixed normalized thresholds, adjacency, and overlap may not permanently delete a candidate before action-aware analysis. Each candidate only reads the two already-built timeline regions immediately before and after it in traversal order, and issues `landingAuthorization` only for proven `airborne→grounded`. An `unknown` side issues nothing; an adjacent or overlapping high-strength window keeps the after side airborne, while a low-strength window entirely below the world-ground threshold does not hide the main landing. Authorization survives later action-weight fade and unqualified micro tails, and is consumed exactly once when applied height crosses from above the ground threshold to ground. Later actual target airborne clears stale authorization. Its normalized threshold is `max(1e-12, 1e-12 / (jumpHeight × actionWeight))`, matching both target-relative zeroing and the world ground threshold. The internal `biped-pet-root-motion-timeline.ts` builds one immutable proof from unique sorted boundaries and support components per continuous sample. Takeoff, touchdown, stale-token, and candidate-side queries reuse one global 512-work-unit budget; side queries only read existing regions and never add scans or reset the budget. Exhausted regions remain `unknown`, issue no new authorization, clear no old authorization, and retain no cross-call hidden state. Loop and ping-pong interior events carry their source window's canonical resolved boundary as a query anchor; loop `duration→0` and ping-pong direction reversals map their sides explicitly. Pause preserves authorization without emitting an impulse; rewind, reset, repeated time, large jumps, pre-touchdown targets, and unauthorized applied crossings emit none. The impulse remains a bounded dimensionless heuristic based on jump intent, action weight, and ending-contributor weight, not a physical collision velocity.
...
- The full `pet-core` suite contains 183 passing tests, including multi-axis `Number.MAX_VALUE` rotation overflow, finite pause/continuous/facing-change fallbacks, direct reset target reuse, caller-owned authorization across action-weight fade and unqualified tails, pause/reset/later-airborne ownership, applied landing delay, dual-threshold micro-windows, adjacent/overlapping low-vs-high strength, equal endpoints, loop/ping-pong directions, decimal-event canonical anchors, seam/turnaround side semantics, ULP boundaries, the 64-window shared budget, and exhausted-budget `unknown` behavior. The versioned `corepack pnpm run test:studio-biped-root-motion-probe` command runs 10,000 seeded stateful cases, 42 forward/reverse ULP touchdowns, and a canonical 64-ballistic-window authorization chain. The 63 tail peaks in that chain stay at or below approximately `2.9999986e-12`, strictly under the `4e-12` world-ground threshold; the main authorization survives until one real applied landing, while a later high-strength window still clears stale authorization. The latest shared-proof observation uses 130 unique boundaries, 64 support components, and 259 of 512 work units; automation locks the semantic structure, `exhausted=false`, and `workUnits<=512`, not the internal work count. Stable categories remain `1707 solved / 8272 clamped / 10 reset / 11 blocked`, 1,169 loop seams, 344 applied landings, and 3,978 brake signals; all 42 ULP probes pass. The new continuous one/64-window observations each report 200 authorization-carrying frames and 100 real impulse consumptions across 2,000 samples. Each run prints local timings but enforces no fragile cross-machine limit. The Three runtime consumer and task-3 VFX implementation are still absent; `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.
