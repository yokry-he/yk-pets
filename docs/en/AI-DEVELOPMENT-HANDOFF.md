...
- `@yk-pets/pet-core` now provides the pure numeric `sampleBipedPetRootMotion` contract. Absolute-time `cumulativeLocal/World` and `cumulativeTurnRadians` remain deterministic targets, while the caller owns only `previousAppliedWorld/previousAppliedTurnRadians`. The solver chases `cumulativeWorld` in world space and derives `appliedLocal/deltaLocal` with the inverse of the current facing, so facing changes cannot rotate or reinterpret already-applied world history. A missing applied state or reset initializes applied to target without velocity or transient events; clamped frames retain target debt.
...
- Applied phase is derived from actual applied world height and the actual vertical delta: ground-threshold height is `grounded`, rising is `takeoff`, falling is `landing`, and stationary positive height is `airborne`. The target composite ballistic signal authorizes touchdown candidates only. A `landingImpulse` is consumed once when the latest valid target transition is a touchdown and applied height subsequently crosses from above the ground threshold to ground. Any actual target airborne interval after that touchdown, including an epsilon crossing inside a window, clears stale authorization. Its normalized threshold is `max(1e-12, 1e-12 / (jumpHeight × actionWeight))`, matching both target-relative zeroing and the world ground threshold; neither a low-intensity micro-window below world ground nor a high-intensity bump still zeroed by the target may clear authorization. Composite-height upper bounds, subdivision, and a fixed node budget keep the query bounded; an unresolved interval conservatively clears stale authorization. Pause, rewind, reset, repeated time, large jumps, pre-touchdown targets, and unauthorized applied crossings emit no impulse. Candidate boundaries still use bounded two-sided probes so overlap, duplication, exact adjacency, and positive gaps cannot manufacture a touchdown. The impulse remains a bounded dimensionless heuristic based on jump intent, action weight, and ending-contributor weight, not a physical collision velocity.
...
- Root Motion normalization, compilation caches, time resolution, and sampling share `normalizeMotionDurationMs`. High-frequency runtimes must normalize at asset compilation and reuse the recursively frozen definition bound to its canonical duration. Public raw/unknown input deliberately pays defensive read, copy, validation, and freeze cost on every call; it does not identity-cache mutable inputs and hide later mutations. A 100,000-sample local comparison recorded canonical/raw at about `48.54/60.12ms` for one window (`1.24×`) and `131.94/502.89ms` for 64 windows (`3.81×`). These are local regression measurements, not cross-machine promises.
- The full `pet-core` suite contains 169 passing tests, including applied landing delay and target authorization, stale-authorization clearing after an in-window target takeoff, dual threshold scaling for low/high-intensity micro-windows, loop/ping-pong directions, pause/reset/rewind/large-jump behavior, facing-change world invariants, representable deltas near `Number.MAX_VALUE`, and bounded authorization/horizontal-support scanning beyond the safe-integer range. A separate 10,000-input seeded stateful probe recorded `5573 solved / 4416 clamped / 11 reset / 10 blocked`, plus 1,037 loop seams, 943 applied landings, and 3,948 brake signals in about `64.81ms`. All 42 representable-interior ULP touchdown probes passed (21 forward and 21 reverse). The Three runtime consumer and task-3 VFX implementation are still absent; `bipedPetRootMotionComplete`, `bipedPetMotionVfxComplete`, and cross-browser GPU acceptance remain false.
