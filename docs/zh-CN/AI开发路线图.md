...
- 状态：Complete; manual browser acceptance required
- 已完成身体部件树、外观式数值面板、当前帧/已选关键帧/整段作用范围、整体与身体缩放、对称编辑和预览拖拽操控板。
- 后续在真实浏览器验收后再实现模型射线拾取与三轴 3D Gizmo。
### 阶段 F.2：动作工坊预览与中文界面优化

- 状态：Complete; manual browser acceptance required
- 已完成默认预览缩小、控制栏连续缩放、三轴自由旋转、拖动操控、视图复位、右侧栏横向溢出修复和中文优先文案；画布滚轮缩放当前暂停。
- 预览变换不写入动作资产；动作本身的整体旋转与缩放继续由语义 Rig 和关键帧承担。
