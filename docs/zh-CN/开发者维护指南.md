...
- `README.zh-CN.md`：产品定位、快速开始和首要文档入口；
- `技术栈.md`：依赖用途、版本基线和选型边界；
- `技术架构.md`：模块职责、依赖方向和数据流；
- `开发与运行指南.md`：开发命令、修改落点和验证矩阵；
- `项目状态.md`：已完成、未完成和下一阶段；
- 专题文档：只描述对应功能、数据结构、操作或验收，不重复整套架构。

`docs/zh-CN/` 中的中文文档使用中文文件名，ADR 保留四位编号并使用中文主题名。`docs/en/` 保留英文文件名。新增文档时必须在 `scripts/check-documentation.mjs` 中登记中英文配对并加入 `docs/README.md` 导航。

重命名文档必须同时更新 README、文档内链接、专项脚本、`.ai` 状态和会话交接。完成后运行：

```bash
pnpm check:documentation
```

该命令会检查中文文件名、必需章节和 Markdown 相对链接，不能通过保留空壳旧文件绕过断链。

## 12. 技术栈变更

升级框架、引入生产依赖或改变构建工具时，需要同步核对：

1. 根目录和受影响 Workspace 的 `package.json`；
2. `pnpm-lock.yaml` 与 Node/pnpm 基线；
3. `技术栈.md` 中的版本、职责和限制；
4. `技术架构.md` 中的依赖方向；
5. `开发与运行指南.md` 和发布文档中的命令；
6. 类型检查、测试、生产构建和真实浏览器验收。

仅更新依赖版本但不核对行为、类型、Bundle 或浏览器兼容性，不算完成技术栈升级。
