...
Three 层复用小型对象池；当前落地环/尘点/拖尾/火花采用 `8/24/8/24` 固定容量，总活动实例不超过 `64`，尘点与火花使用 `InstancedMesh`。每类默认只创建一份 Geometry/Material，单个 burst 不超过 `16` 个粒子、拖尾不超过 `8` 个、寿命不超过 `1200ms`；64 个 slot 状态预分配复用，同到期实例按激活先后公平轮转，256 项近期 ID 使用固定环形队列且仍活动的 burst 不会被淘汰后重放。burst 捕获共同父级中的触发位置，sustain 才随同一 ID 更新位置；朝向遵循 Root Motion 局部 `+Z`，世界前向为 `(sin(yaw), 0, cos(yaw))`。对象池只由安全复制的动作 `requestedTimeMs`、位置和朝向推进，不读取墙钟或创建 RAF；reset 清活动状态但保留池。自定义工厂共享的资源按对象身份只释放一次，默认回调多次调用也只产生一个 provisional；控制器跟踪并移除挂到任意父级的全部自有子节点。dispose 将活动清理、GPU 资源与父级解绑逐项尽力且每个资源只尝试一次。第一阶段不实现体积烟雾、后处理 Bloom、屏幕震动或音画时间轴编辑。
...
本阶段实现混合 Root Motion 基础、行走/冲刺/跳跃运行链路、自动重心协同和四种确定性运动 VFX。VFX 释放入口必须在资源派发同步 `dispose` 事件前封存重入，使监听器无法重复释放同一 GPU 资源或写入正在清理的池。
