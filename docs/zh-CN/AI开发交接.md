...
- 确定性后备不再依赖连续前缀/后缀能否聚合为一个两段三角形。它在 Root→Target/Pole 二维平面逐段选择下一节点到目标的剩余距离，对“当前圆约束区间”和“剩余后缀可达区间”取交集；每层按边界、目标保留值及 2→64 等分逐级细化候选，尝试两个圆交点并剪除 Pole 负半平面分支，最后映射回三维正交基复核全部后置条件。
...
- 一般构造面向当前短链设置 `16384` 个搜索节点的明确预算；预算耗尽、坐标精度不足或后置复核失败时安全返回 `blocked`。因此文档承诺的是在该预算内支持完整物理距离域的确定性短链构造，不对任意长链作无边界的绝对完备性声明。
