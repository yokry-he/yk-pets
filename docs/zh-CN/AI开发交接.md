...
- 弹道高度继续使用 `4 × jumpHeight × characterHeight × p × (1-p)`。阶段改由实际 applied 世界高度与本帧实际纵向增量判断：地面阈值内为 `grounded`，上升为 `takeoff`，下降为 `landing`，正高度静止为 `airborne`。所有合法 `ballistic` 窗口的正向 end 与反向 start 都先保留为候选，同边界贡献权重只聚合一次；固定归一化阈值、相邻和重叠关系不得在 action-aware 分析前永久删除候选。每个候选只读取单次共享时间线中遍历方向 before/after 两个紧邻区域，且必须同时得到 proven `airborne→grounded` 才签发 `landingAuthorization`；任一侧 `unknown`、或相邻/重叠高强度窗让 after 仍为 `airborne` 时均不签发，相同位置的低强度窗若低于世界接地阈值则不遮蔽主窗 touchdown。授权跨后续 `actionWeight` 淡出与不合格微尾窗保持，applied 高度随后从阈值上方真实跨到地面时只消费一次并输出 `landingImpulse`。后续实际 target airborne 清除 stale 授权；归一化判断阈值取 `max(1e-12, 1e-12 / (jumpHeight × actionWeight))`，同时匹配 target 自身相对零化和实际世界接地。内部 `biped-pet-root-motion-timeline.ts` 每次连续采样按唯一排序边界与支撑分量构建一份不可变证明，takeoff、touchdown、stale 与全部候选双侧查询复用同一个 `512` work-unit 全局预算；查询只读取已构建 region，不新增扫描或预算重置。预算耗尽的区域保持 `unknown`，不签发新授权、不清除旧授权，也没有跨调用隐藏状态。loop 的 `duration→0` 周期缝与 ping-pong 转折方向均显式映射。精确边界上的 applied touchdown 仍可消费该次授权；暂停保留授权但不触发，倒退、reset、同时间重复、大跳、target 尚未合格 touchdown 或未授权的 applied 越地均不触发。冲量仍是跳高意图、动作权重和结束贡献窗权重占比所得的 `[0,1]` 无量纲启发式强度。
...
- 单帧世界位移长度限制为角色高度的 `0.25`，单帧转向限制为 `π/4`；target 追赶误差和由当前朝向转换到世界空间的 `footResidual` 合成后只进行一次世界向量钳制。超过任一预算返回 `clamped`，只限制 applied，不污染 cumulative target。`deltaWorld` 使用 JavaScript 实际可表示的 `appliedWorld - previousAppliedWorld` 重新计算，`deltaLocal` 再由当前朝向逆变换；实际加法在 `Number.MAX_VALUE` 附近未改变位置时，delta 与速度稳定为零。转向增量与角速度采用同样的实际 applied 差值原则。原地、窗口空隙、仅弹道、暂停、reset 与 `actionWeight→0` 不漂移；纯函数不保存低通历史，滤波所有权留给任务 5 控制器。
...
- 自动测试增至 183 项，并新增 `Number.MAX_VALUE` 多轴旋转溢出、暂停/连续/换朝向全有限输出、reset 直接复用 target、caller-owned 授权跨权重淡出与不合格尾窗、暂停保留/reset 清除/真实再腾空清除、applied 落地延迟、低/高强度微窗双层阈值、相邻/重叠低高强度、同终点、loop/ping-pong 正反向、十进制事件 canonical 锚点、周期缝/转折点侧别、ULP、64 窗共享预算及预算耗尽 `unknown` 回归。版本化命令 `corepack pnpm run test:studio-biped-root-motion-probe` 固定执行 10,000 组有状态输入、42 个正反向 ULP touchdown 和 canonical 64 弹道窗授权链；64 窗场景的 63 个尾窗峰值最高约 `2.9999986e-12`，严格低于 `4e-12` 世界接地阈值，主窗授权保持到 applied 真实落地并只消费一次，后续高强度窗仍会清除 stale 授权。该次共享分析观测为 `130` 个唯一边界、`64` 个支撑分量和 `259/512` work unit；自动门禁只锁定语义构造、`exhausted=false` 与 `workUnits<=512`，不锁内部实现计数。固定种子分类仍为 `1707 solved / 8272 clamped / 10 reset / 11 blocked`，另命中 `1169` 次 loop 接缝、`344` 次 applied landing 与 `3978` 次 brake，42 个 ULP 探针全部通过。原每类 100,000 次 1/64 窗 raw/canonical reset 对照明确只观测防御规范化成本；新增每类 2,000 帧的 1/64 窗 canonical 连续弹道热路径，逐帧携带 previous applied、turn 与 authorization，两类均观察到 `200` 个授权帧和 `100` 次真实消费。脚本打印本机耗时但不设置跨机器脆弱阈值。
- 本批仍未让 Three 运行时容器消费这些累计/增量结果，也未实现任务 3 的确定性 VFX 信号消费或对象池。`.ai/project-state.json` 的完成项包含 `biped-pet-root-motion-solver`、`biped-pet-root-motion-shared-ballistic-timeline` 与本次 `biped-pet-root-motion-action-aware-boundaries`；`bipedPetRootMotionComplete`、`bipedPetMotionVfxComplete` 与跨浏览器 GPU 验收继续保持 `false`。
