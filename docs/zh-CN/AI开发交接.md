...
- 标准三至四骨骼腿链的 `auto` 路径优先使用解析式 Two Bone IK；显式 `fabrik` 或更长的非标准链使用受约束 FABRIK。世界方向修正会先转换到父级局部空间再写入 Quaternion，并按 Profile 权重、动作权重、接触阶段权重、置信度和单帧最大修正角共同限制。
- 双支撑阶段会先根据两脚垂直锚点误差修正骨盆局部 Y，严格钳制在绑定值的 `[-0.08, 0.08]` 范围；单支撑不启用该骨盆策略。腿链修正后保持脚部 FK 世界朝向，并在刚性脚部末端消除剩余位置误差，连续 locked 帧的足底世界误差低于 `1e-3`。
...
- 自动测试使用真实 Three `Bone` 覆盖左右脚独立锁定、时间与 Clip 切换清锁、解析式/FABRIK 路由、损坏单肢回退、双/单支撑骨盆边界、零权重、输入不突变、有限单位 Quaternion、重置和重复释放。本批不实现完整 Root Motion、地形法线检测、动作特效或正式四足/机甲 Profile；`bipedPetRootMotionComplete` 与 `bipedPetMotionVfxComplete` 继续保持 `false`。
