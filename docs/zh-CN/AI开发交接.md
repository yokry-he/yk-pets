...
- 弹道高度继续使用 `4 × jumpHeight × characterHeight × p × (1-p)`。阶段改由实际 applied 世界高度与本帧实际纵向增量判断：地面阈值内为 `grounded`，上升为 `takeoff`，下降为 `landing`，正高度静止为 `airborne`。target 合成弹道的 touchdown 候选必须先在当前有效强度下证明真实 airborne→grounded，才可签发 `landingAuthorization`；授权跨后续 `actionWeight` 淡出与不合格微尾窗保持，applied 高度随后从阈值上方真实跨到地面时只消费一次并输出 `landingImpulse`。后续实际 target airborne 清除 stale 授权；归一化判断阈值取 `max(1e-12, 1e-12 / (jumpHeight × actionWeight))`，同时匹配 target 自身相对零化和实际世界接地，并通过区间上界、二分和固定节点预算保持有界。低强度微窗始终低于世界接地阈值、或高强度微窗仍被 target 相对阈值归零时均不覆盖或清除授权。精确边界上的 applied touchdown 仍可消费该次授权；暂停保留授权但不触发，倒退、reset、同时间重复、大跳、target 尚未合格 touchdown 或未授权的 applied 越地均不触发。候选边界继续经过有界双侧探测，避免重叠、重复、相邻和正 gap 制造伪 touchdown；冲量仍是跳高意图、动作权重和结束贡献窗权重占比所得的 `[0,1]` 无量纲启发式强度。
...
- 自动测试增至 173 项，并新增 `Number.MAX_VALUE` 多轴旋转溢出、暂停/连续/换朝向全有限输出、reset 直接复用 target、caller-owned 授权跨权重淡出与不合格尾窗、暂停保留/reset 清除/真实再腾空清除、applied 落地延迟、低/高强度微窗双层阈值、loop/ping-pong 正反向与 ULP 回归。版本化命令 `corepack pnpm run test:studio-biped-root-motion-probe` 固定执行 10,000 组有状态输入、42 个正反向 ULP touchdown 和每类 100,000 次 raw/canonical 性能观测；有状态分类稳定为 `1707 solved / 8272 clamped / 10 reset / 11 blocked`，另命中 `1169` 次 loop 接缝、`344` 次 applied landing 与 `3978` 次 brake，42 个 ULP 探针全部通过。脚本逐次打印单窗口与 64 窗口 canonical/raw 本机耗时和校验和，只断言有限、确定、分类命中、ULP 正确性与 raw/canonical 结果一致，不设置跨机器脆弱耗时阈值。
