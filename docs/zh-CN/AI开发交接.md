...
- 弹道高度继续使用 `4 × jumpHeight × characterHeight × p × (1-p)`。阶段改由实际 applied 世界高度与本帧实际纵向增量判断：地面阈值内为 `grounded`，上升为 `takeoff`，下降为 `landing`，正高度静止为 `airborne`。target 合成弹道只授权 touchdown 候选；只有最近的有效 target 转换是 touchdown，且 applied 高度随后从阈值上方真实跨到地面时，才消费一次 `landingImpulse`。touchdown 后任何实际 target airborne（包括窗口内部越过稳定零阈值）都会清除 stale 授权；归一化判断阈值取 `max(1e-12, 1e-12 / (jumpHeight × actionWeight))`，同时匹配 target 自身相对零化和实际世界接地，并通过区间上界、二分和固定节点预算保持有界，无法证明持续 grounded 时保守清除旧授权。低强度微窗始终低于世界接地阈值、或高强度微窗仍被 target 相对阈值归零时均不清授权。精确边界上的 applied touchdown 仍可消费该次授权；暂停、倒退、reset、同时间重复、大跳、target 尚未 touchdown 或未授权的 applied 越地均不触发。候选边界继续经过有界双侧探测，避免重叠、重复、相邻和正 gap 制造伪 touchdown；冲量仍是跳高意图、动作权重和结束贡献窗权重占比所得的 `[0,1]` 无量纲启发式强度。
...
- 自动测试增至 169 项，并新增 applied 落地延迟、目标授权、窗内 target takeoff 的 stale 授权清除、低/高强度微窗双层阈值、loop/ping-pong 正反向、暂停/reset/回拖/大跳、朝向变化世界不变量、`Number.MAX_VALUE` 实际差值，以及超过安全整数范围的授权与水平支撑扫描有界性回归。独立固定种子 10,000 组有状态输入探针结果为 `5573 solved / 4416 clamped / 11 reset / 10 blocked`，另命中 `1037` 次 loop 接缝、`943` 次 applied landing 与 `3948` 次 brake；本机耗时约 `64.81ms`，平均约 `0.00648ms/有效输入`。42 个可表示内部点 ULP 级落地探针（正向 21、反向 21）全部通过。100,000 次 raw/canonical 对照微基准：单窗口 canonical 约 `48.54ms`、raw 约 `60.12ms`（raw/canonical `1.24×`）；64 窗口 canonical 约 `131.94ms`、raw 约 `502.89ms`（`3.81×`）。这些数值只记录本机回归量级，不作为跨机器性能承诺。
