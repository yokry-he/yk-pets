...
- 自动验证已通过 `corepack pnpm --filter @yk-pets/pet-core test`（142 项）、`corepack pnpm --filter @yk-pets/pet-core typecheck`、10,000 长度资源预算探针、编译 Clip 改时长与跨 Clip 复用的两个独立时长探针、`node scripts/check-ai-handoff.mjs`、`node scripts/check-documentation.mjs` 和 `git diff --check`。
