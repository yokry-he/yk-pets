...
- 弹道高度继续使用 `4 × jumpHeight × characterHeight × p × (1-p)`。阶段改由实际 applied 世界高度与本帧实际纵向增量判断：地面阈值内为 `grounded`，上升为 `takeoff`，下降为 `landing`，正高度静止为 `airborne`。target 合成弹道的 touchdown 候选必须先在当前有效强度下证明真实 airborne→grounded，才可签发 `landingAuthorization`；授权跨后续 `actionWeight` 淡出与不合格微尾窗保持，applied 高度随后从阈值上方真实跨到地面时只消费一次并输出 `landingImpulse`。后续实际 target airborne 清除 stale 授权；归一化判断阈值取 `max(1e-12, 1e-12 / (jumpHeight × actionWeight))`，同时匹配 target 自身相对零化和实际世界接地。内部 `biped-pet-root-motion-timeline.ts` 每次连续采样按唯一排序边界与支撑分量构建一份不可变证明，takeoff、touchdown 与 stale 查询复用同一个 `512` work-unit 全局预算；预算耗尽的范围保持 `unknown`，不签发新授权、不清除旧授权，也没有跨调用隐藏状态。低强度微窗始终低于世界接地阈值、或高强度微窗仍被 target 相对阈值归零时均不覆盖或清除授权。精确边界上的 applied touchdown 仍可消费该次授权；暂停保留授权但不触发，倒退、reset、同时间重复、大跳、target 尚未合格 touchdown 或未授权的 applied 越地均不触发。候选边界继续经过有界双侧探测，避免重叠、重复、相邻和正 gap 制造伪 touchdown；冲量仍是跳高意图、动作权重和结束贡献窗权重占比所得的 `[0,1]` 无量纲启发式强度。
...
- 自动测试增至 180 项，并新增 `Number.MAX_VALUE` 多轴旋转溢出、暂停/连续/换朝向全有限输出、reset 直接复用 target、caller-owned 授权跨权重淡出与不合格尾窗、暂停保留/reset 清除/真实再腾空清除、applied 落地延迟、低/高强度微窗双层阈值、loop/ping-pong 正反向、十进制事件 canonical 锚点、周期缝/转折点侧别、ULP、64 窗共享预算及预算耗尽 `unknown` 回归。版本化命令 `corepack pnpm run test:studio-biped-root-motion-probe` 固定执行 10,000 组有状态输入、42 个正反向 ULP touchdown、canonical 64 弹道窗授权链和每类 100,000 次 raw/canonical 性能观测；64 窗场景的 63 个尾窗峰值最高约 `2.9999986e-12`，严格低于 `4e-12` 世界接地阈值，主窗授权保持到 applied 真实落地并只消费一次，后续高强度窗仍会清除 stale 授权。其共享分析固定为 `130` 个唯一边界、`64` 个支撑分量和 `259/512` work unit。固定种子分类仍为 `1707 solved / 8272 clamped / 10 reset / 11 blocked`，另命中 `1169` 次 loop 接缝、`344` 次 applied landing 与 `3978` 次 brake，42 个 ULP 探针全部通过。脚本逐次打印共享分析统计、单窗口与 64 窗口 canonical/raw 本机耗时和校验和，只断言有限、确定、分类命中、预算上限、授权与 ULP 正确性及 raw/canonical 结果一致，不设置跨机器脆弱耗时阈值。
- 本批仍未让 Three 运行时容器消费这些累计/增量结果，也未实现任务 3 的确定性 VFX 信号消费或对象池。`.ai/project-state.json` 的完成项包含 `biped-pet-root-motion-solver` 与本次 `biped-pet-root-motion-shared-ballistic-timeline`；`bipedPetRootMotionComplete`、`bipedPetMotionVfxComplete` 与跨浏览器 GPU 验收继续保持 `false`。
