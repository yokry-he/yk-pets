...
- 末端命中不再单独构成成功条件：无可行聚合分割的非共面链即使 FABRIK 在 3 次迭代后把末端误差降为零，只要任一内部节点仍脱离统一求解平面，就会保留实际迭代次数与有限姿态并返回 `blocked`。
...
- 确定性后备不再依赖连续前缀/后缀能否聚合为一个两段三角形。它在 Root→Target/Pole 二维平面逐段选择下一节点到目标的剩余距离，对“当前圆约束区间”和“剩余后缀可达区间”取交集，再使用圆圆交点选择 Pole 正半平面的候选；最后映射回三维正交基并复核末端、段长、共面和半平面约束。
