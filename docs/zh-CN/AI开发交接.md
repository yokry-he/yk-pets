...
- 首帧、停止、回拖、Clip 切换、求解 reset、runtime 重建和 dispose 会清除旧 applied、欠量与落地授权；暂停保持 applied 与待消费授权但不移动、不发信号。粗帧跨越 canonical touchdown 与在边界细分帧都只消费一次同强度冲量；Three 层不重建授权、不吸附窗口端点、不改写 `sqrt(normalizedCompositePeakHeight)` 强度。动作控制器每帧返回当前 Root Motion、确定性 VFX 信号、深冻结 IK 数值报告，以及本帧消费/下一帧待用的有限局部水平残差，Y 固定为零；`residualByLimb` 明确定义为每肢 `anchor-current` 的世界 X/Z `hypot` 幅值，禁止三维距离让纯 Y 误差渗入水平 feedback。原地、travel 空隙和腾空会清掉残差所有权而不漂移。
- 新增独立 `ComplexBipedBalanceController`，固定只拥有 pelvis X/Z 和 chest Quaternion；水平向量长度不超过 `characterHeight×0.025`，胸部倾斜不超过 `0.12rad`。它能在 standalone 连续调用中识别并移除自己上一帧的 tilt，同时不会反向修改外层已经写入的新 FK；双支撑取中心、单支撑按实际运动强度偏向支撑侧，腾空、reset 和 dispose 恢复自身绑定所有权。
- IK `apply()` 现在返回深冻结且不含 Bone 的 `supportingContacts/residualByLimb/clampedLimbs`，并接受完整链可选传入的只读 `rootMotionPhase` 上下文。实际 applied 相位为 `takeoff/airborne` 时，相位优先于资产中可能滞后的非零 contact weight：IK 先恢复 pelvis 绑定 Y、释放旧锚并返回空支撑报告；standalone 不传上下文时保持历史接触语义。直接创建 IK 控制器的历史行为不变：单支撑仍不写 pelvis Y、仍用三轮交替，`100→320ms` 的约 `0.014` 不可达残差继续报告 clamped。真实可达域扫描证明，在容器已经写入行走 Root Motion、腿段不可伸长且 balance X/Z 受 `0.025×height` 限制时，允许圆盘内的最佳世界残差仍约 `0.00825`，无法满足集成链 `<1e-3`。因此只有完整动作控制器显式开启尺寸化单支撑 pelvis Y 策略，预算为 `min(0.08, characterHeight×0.025)`；集成链在相同每骨骼累计角预算内使用五轮交替收敛，固定 `100→320ms` 探针的世界残差由 `0.000999317248683272` 降至约 `0.00031089757658926703`，门禁为 `≤0.00075`。pelvis Y 仍由 IK 独占，达到预算会在报告和诊断中明确 clamped，腾空、无支撑、weight 0、reset 与 dispose 恢复绑定 Y，不拉伸腿段或改写 foot/ankle 局部 position。
- 外层顺序固定为 `restoreBindPose → FK → Root Motion → Balance → updateMatrixWorld → IK`。自动测试覆盖不同帧率同目标 applied、巨大 target 追债、暂停授权、停止/回拖/Clip 切换、世界轴转向与非零绑定姿态、角色尺寸步幅、粗/细 touchdown、双/单支撑与腾空、滞后 contact 的真实 takeoff/airborne 释放、纯 Y 残差隔离、默认关闭/集成开启的尺寸化 pelvis Y、三轮/五轮且总角预算不变、`≤7.5e-4` 集成残差裕量、blocked Root Motion 下 FK/IK 继续、IK 报告与诊断冻结、残差跨帧及 gap 清理、脚局部 position/段长、重复释放和释放后拒绝写入。当前只完成 Three 控制器层，正式 renderer 尚未接入该返回帧，VFX 对象池也未实现；所以 `bipedPetRootMotionThreeRuntimeComplete=true`，但 `bipedPetRootMotionComplete`、`bipedPetMotionVfxComplete` 与跨浏览器 GPU 验收继续为 `false`。
