...
export interface StudioNumericControlDefinition {
  path: string; label: string; section: StudioControlSection; defaultValue: number
  recommendedRange: readonly [number, number]; hardRange: readonly [number, number]; step: number; rendererConsumer: string
}
const control = (path:string,label:string,section:StudioControlSection,defaultValue:number,recommendedRange:readonly[number,number],hardRange:readonly[number,number],step:number,rendererConsumer:string):StudioNumericControlDefinition => Object.freeze({path,label,section,defaultValue,recommendedRange,hardRange,step,rendererConsumer})
export const STUDIO_CONTROL_REGISTRY = Object.freeze({
  ...
  'customization.nose.offsetX':control('customization.nose.offsetX','鼻子左右位置','face',0,[-.1,.1],[-.35,.35],.01,'ExtensionCloudFoxFaceCustomization'),
  'customization.nose.offsetY':control('customization.nose.offsetY','鼻子上下位置','face',0,[-.1,.1],[-.35,.35],.01,'ExtensionCloudFoxFaceCustomization'),
  'customization.nose.surfaceOffset':control('customization.nose.surfaceOffset','鼻子表面深度','face',.012,[-.01,.04],[-.05,.12],.002,'ExtensionCloudFoxFaceCustomization'),
  'customization.nose.scaleX':control('customization.nose.scaleX','鼻子宽度','face',1,[.72,1.3],[.35,2],.01,'ExtensionCloudFoxFaceCustomization'),
  'customization.nose.scaleY':control('customization.nose.scaleY','鼻子高度','face',1,[.72,1.3],[.35,2],.01,'ExtensionCloudFoxFaceCustomization'),
  'customization.nose.scaleZ':control('customization.nose.scaleZ','鼻子厚度','face',1,[.72,1.3],[.35,2],.01,'ExtensionCloudFoxFaceCustomization'),
  'customization.nose.rotation':control('customization.nose.rotation','鼻子旋转','face',0,[-.35,.35],[-.8,.8],.01,'ExtensionCloudFoxFaceCustomization'),
...
export function isStudioValueOutsideRecommended(path:StudioControlPath,value:number){const[min,max]=STUDIO_CONTROL_REGISTRY[path].recommendedRange;return value<min||value>max}
