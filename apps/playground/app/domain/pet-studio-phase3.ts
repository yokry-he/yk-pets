...
export function auditPetStudioAppearance(recipe: PetStudioAppearanceRecipe): AppearanceAuditFinding[] {
  ...
  const tailLength = recipe.tailDesign.rootExtensionLength + recipe.tailDesign.segments.reduce((sum, segment) => sum + segment.length, 0)
  ...
  if (Math.abs(recipe.tailDesign.lateralOffset) > 1) findings.push({ id: 'tail-lateral-bounds', severity: 'info', message: '尾巴整体侧偏较大，建议切换到背面视角检查根段是否自然。', path: 'tailDesign.lateralOffset' })
  if (recipe.tailDesign.rootExtensionLength < .18) findings.push({ id: 'tail-root-short', severity: 'warning', message: '背部根段过短，尾巴可能看起来直接插在身体侧面。', path: 'tailDesign.rootExtensionLength' })
  if (recipe.tailDesign.segments.some(segment => Math.max(Math.abs(segment.offsetX), Math.abs(segment.offsetY), Math.abs(segment.offsetZ)) > .48)) findings.push({ id: 'tail-segment-offset', severity: 'warning', message: '某一尾段偏移接近安全边界，请检查段与段之间是否断开。', path: 'tailDesign.segments' })
