...
<template>
  <section class="prop-workspace">
    ...
    <div class="editor-area">
      <header class="editor-header"><div><small>PROP STUDIO</small><h2>{{ selected?.nameZh || '请选择或创建道具' }}</h2><span v-if="selected">schema v{{ selected.schemaVersion }} · {{ selected.components.length }}/48 components</span></div><div class="view-buttons"><button v-for="item in ['front','left','back','right'] as const" :key="item" :class="{active:session.previewView===item}" @click="setView(item)">{{ item }}</button></div></header>
      <div class="canvas-grid">
        <div class="pet-preview"><ClientOnly><CloudFoxStudioCanvas :appearance="appearance.recipe" behavior="idle" :motion-key="selected?.updatedAt || 0" :view="session.previewView" :background="session.previewBackground" focus="full" :prop-instances="previewInstances" :prop-assets="selected ? [selected] : []" preserve-prop-materials /></ClientOnly><span class="anchor-badge">挂载预览：{{ selected?.defaultAnchor || '未选择' }}</span></div>
