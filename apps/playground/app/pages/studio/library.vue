...
const complexStatusLabel = computed(() => {
  const complex = currentVariants.value.complex
  if (complex.status === 'ready') return '就绪'
  if (complex.status === 'blocked') return '需修复'
  if (complex.status === 'draft') return `草稿 ${complex.completion}%`
  return '未创建'
})
...
<template>
  <section class="library-page">
    ...
    <section class="asset-section appearance-card">
      ...
      <div class="appearance-info"><strong>{{ appearance.recipe.identity.nameZh }} / {{ appearance.recipe.identity.nameEn }}</strong><span>{{ appearance.recipe.parts.headShape }} · {{ appearance.recipe.parts.bodyShape }}</span><div class="model-variant-badges" aria-label="模型变体状态"><span class="model-variant-badge ready">简单模型 <b class="model-variant-value">就绪</b></span><span class="model-variant-badge" :data-status="currentVariants.complex.status">复杂模型 <b class="model-variant-value">{{ complexStatusLabel }}</b></span></div><code>{{ session.selectedAppearanceId }}</code></div>
    </section>
