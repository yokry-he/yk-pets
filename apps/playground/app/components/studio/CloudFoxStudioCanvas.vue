...
import type { PetStudioAppearanceRecipe, CloudFoxStudioBackground, CloudFoxStudioBehavior, CloudFoxStudioView } from '~/domain/pet-studio-phase2'
...
const petBounds = computed(()=>calculatePetVisualBounds(props.appearance)) // pet-only bounds; scene/background is intentionally excluded
...
<template>
  <div :class="`studio-canvas studio-canvas--${background}`">
    ...
    <div class="label"><strong>{{ appearance.identity.nameZh }} · {{ appearance.identity.nameEn }}</strong><span>自动镜头 · {{ petBounds.width.toFixed(1) }} × {{ petBounds.height.toFixed(1) }}</span></div>
  </div>
</template>
<style scoped>
