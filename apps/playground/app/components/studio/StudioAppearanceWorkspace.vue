...
const {
  ...
  endPreviewRotate,
  cancelPreviewRotate,
} = useStudioPreviewOrientation({ excludedSelector: '.part-hotspots' })
...
watch(tab,next=>{
  if (['face','body','limbs','belly','antenna','colors'].includes(next)) setView('front')
  if (next==='tail') setView('left')
