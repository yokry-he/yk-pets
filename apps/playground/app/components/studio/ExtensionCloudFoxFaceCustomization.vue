<!--
  ...
  在唯一头部局部坐标中渲染可配置鼻子、经典/猫系/线条/张嘴/嘟嘴和动态脸颊，不再使用遮挡层覆盖旧部件。
  Renders configurable noses, classic/cat/line/open/pout mouths, and animated cheeks in the sole head-local space without cover meshes over legacy parts.
-->
...
const nosePosition = computed(() => vector([
  ...
  scheme.model.head.nosePosition[2] + .012,
]))
const mouthPosition = computed(() => vector([
  scheme.model.head.mouthPosition[0],
  scheme.model.head.mouthPosition[1],
  scheme.model.head.mouthPosition[2] + (props.appearance.parts.mouth === 'smile' ? 0 : .012),
]))
...
const smileLeft = new CatmullRomCurve3([
  vector([-.19, .025, .012]),
  vector([-.14, -.035, .018]),
  vector([-.075, -.07, .022]),
  vector([0, -.062, .024]),
])
const smileRight = new CatmullRomCurve3([
  vector([0, -.062, .024]),
  vector([.075, -.07, .022]),
  vector([.14, -.035, .018]),
  vector([.19, .025, .012]),
])
const catLeft = new CatmullRomCurve3([
  vector([-.18, .018, .012]),
  vector([-.12, -.07, .018]),
  vector([-.055, -.085, .022]),
  vector([0, -.025, .024]),
])
const catRight = new CatmullRomCurve3([
  vector([0, -.025, .024]),
  vector([.055, -.085, .022]),
  vector([.12, -.07, .018]),
  vector([.18, .018, .012]),
])
const poutCurve = new CatmullRomCurve3(
...
<template>
  <TresGroup>
    <TresGroup :position="nosePosition" :scale="noseScale">
      ...
      <template v-else-if="appearance.parts.nose === 'button'">
        <TresMesh :rotation="rotation([Math.PI / 2, 0, 0])">
          <TresCylinderGeometry :args="[.72, .72, .34, 28]" />
          <TresMeshStandardMaterial :color="colors.nose" :roughness=".28" />
        </TresMesh>
        <TresMesh v-for="side in [-1, 1]" :key="side" :position="vector([side * .24, 0, .2])" :scale="vector([.09, .09, .06])">
          <TresSphereGeometry />
          <TresMeshBasicMaterial :color="colors.muzzle" />
        </TresMesh>
      </template>
      <template v-else-if="appearance.parts.nose === 'heart'">
        <TresMesh v-for="side in [-1, 1]" :key="side" :position="vector([side * .32, .16, 0])" :scale="vector([.56, .56, .66])">
          <TresSphereGeometry />
          <TresMeshStandardMaterial :color="colors.nose" :roughness=".2" />
        </TresMesh>
        <TresMesh :position="vector([0, -.3, 0])" :rotation="rotation([0, 0, Math.PI])" :scale="vector([.68, .68, .7])">
          <TresConeGeometry :args="[.7, 1.05, 3]" />
          <TresMeshStandardMaterial :color="colors.nose" :roughness=".2" />
        </TresMesh>
      </template>
      <TresMesh v-else>
    ...
    <TresMesh
      v-for="side in [-1, 1]"
      :key="`face-cheek-${side}`"
      :position="vector([side * scheme.model.head.cheekOffset[0], scheme.model.head.cheekOffset[1], scheme.model.head.cheekOffset[2] + .015])"
      :scale="vector(scheme.model.head.cheekScale)"
    >
      <TresSphereGeometry />
      <TresMeshBasicMaterial :ref="registerCheek" :color="colors.cheeks" transparent :opacity="0" :depth-write="false" />
    </TresMesh>
    ...
    <TresGroup ref="mouth" :position="mouthPosition">
      <template v-if="appearance.parts.mouth === 'smile'">
        <TresMesh :scale="vector(scheme.model.head.mouthScale)">
          <TresSphereGeometry />
          <TresMeshStandardMaterial :color="colors.mouth" :roughness=".2" />
        </TresMesh>
      ...
      <TresMesh v-else-if="appearance.parts.mouth === 'line'" :scale="vector([.3, .018, .018])">
        <TresBoxGeometry />
        <TresMeshBasicMaterial :color="colors.mouth" />
      </TresMesh>
      <template v-else-if="appearance.parts.mouth === 'open'">
        <TresMesh :scale="vector([.22, .28, .055])">
          <TresSphereGeometry :args="[1, 28, 24]" />
          <TresMeshStandardMaterial :color="colors.mouth" :roughness=".25" />
        </TresMesh>
        <TresMesh :position="vector([0, -.09, .052])" :scale="vector([.14, .09, .025])">
          <TresSphereGeometry :args="[1, 24, 20]" />
          <TresMeshBasicMaterial :color="colors.tongue" />
        </TresMesh>
      </template>
      <TresMesh v-else-if="appearance.parts.mouth === 'pout'">
        <TresTubeGeometry :args="[poutCurve, 24, .017, 8, true]" />
        <TresMeshBasicMaterial :color="colors.mouth" />
      </TresMesh>
      <template v-else>
