<!--
  ...
  在唯一头部局部空间中渲染鼻子、经典/猫系/线条/张嘴/嘟嘴和动态脸颊；只读取独立头型 Profile。
  Renders noses, classic/cat/line/open/pout mouths, and animated cheeks in the sole head-local space using only the independent head profile.
-->
...
const nosePosition = computed(() => vector([
  (scheme.model.head.nosePosition[0] + profile.value.muzzleOffset[0]) * headScale.value,
  (scheme.model.head.nosePosition[1] + profile.value.muzzleOffset[1]) * headScale.value,
  (scheme.model.head.nosePosition[2] + .012 + profile.value.faceDepth) * headScale.value,
]))
const mouthPosition = computed(() => vector([
  (scheme.model.head.mouthPosition[0] + profile.value.muzzleOffset[0]) * headScale.value,
  (scheme.model.head.mouthPosition[1] + profile.value.muzzleOffset[1]) * headScale.value,
  (scheme.model.head.mouthPosition[2] + (props.appearance.parts.mouth === 'smile' ? 0 : .012) + profile.value.faceDepth) * headScale.value,
]))
const noseScale = computed(() => {
  ...
  return vector(scheme.model.head.noseScale.map(value => value * factor))
})
const cheekPosition = (side: number) => vector([
  (side * scheme.model.head.cheekOffset[0] + profile.value.muzzleOffset[0]) * headScale.value,
  (scheme.model.head.cheekOffset[1] + profile.value.muzzleOffset[1]) * headScale.value,
  (scheme.model.head.cheekOffset[2] + .015 + profile.value.faceDepth) * headScale.value,
])
const cheekScale = computed(() => vector(scheme.model.head.cheekScale.map(value => value * headScale.value)))
const mouthGeometryScale = computed(() => headScale.value)
...
<template>
  <TresGroup>
    ...
    <TresMesh v-for="side in [-1, 1]" :key="`face-cheek-${side}`" :position="cheekPosition(side)" :scale="cheekScale">
      <TresSphereGeometry /><TresMeshBasicMaterial :ref="registerCheek" :color="colors.cheeks" transparent :opacity="0" :depth-write="false" />
    </TresMesh>

    <TresGroup ref="mouth" :position="mouthPosition" :scale="vector([mouthGeometryScale, mouthGeometryScale, mouthGeometryScale])">
