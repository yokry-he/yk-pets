...
<template>
  <section class="prop-events">
    <header><div><small>PROP EVENTS</small><h3>道具事件轨道</h3></div><span>{{ Math.round(playheadTimeMs) }} ms</span></header>
    <div v-if="propAssets.length" class="event-form">
      ...
      <input v-model="instanceId" aria-label="实例 ID" placeholder="instance id">
      <select v-model="kind"><option v-for="item in ['create','show','attach','detach','move','hide','style','destroy']" :key="item" :value="item">{{ item }}</option></select>
    ...
    <div class="event-list">
      <button v-for="event in events" :key="event.id" :class="{ selected: selectedEventIds.includes(event.id) }" @click="emit('select', event.id, $event.ctrlKey || $event.metaKey)">
        <b>{{ event.timeMs }}</b><span>{{ event.kind }}</span><small>{{ event.instanceId }} · {{ event.propId }}</small>
      </button>
    </div>
...
<style scoped>
.prop-events{display:grid;gap:7px;padding:9px;border:1px solid #ffffff12;border-radius:11px;background:#070b15}.prop-events header{display:flex;align-items:center;justify-content:space-between}.prop-events header div{display:grid;gap:3px}.prop-events small{color:#727d9d;font-size:8px}.prop-events h3,.prop-events p{margin:0}.prop-events header span{font:700 9px/1 ui-monospace,monospace;color:#66e8ff}.event-form{display:grid;gap:5px}.event-form input,.event-form select,.event-form button,.delete{min-height:30px;border:1px solid #ffffff1b;border-radius:7px;color:#eef2ff;background:#0a1020}.vector,.style-row{display:grid;grid-template-columns:repeat(3,1fr);gap:4px}.style-row input[type=color]{width:100%;padding:3px}.event-list{display:grid;gap:4px;max-height:140px;overflow:auto}.event-list button{display:grid;grid-template-columns:42px 52px 1fr;align-items:center;gap:5px;min-height:29px;border:1px solid #ffffff12;border-radius:7px;color:#b8c1dc;background:#ffffff04;text-align:left}.event-list button.selected{border-color:#ffcb6b77;background:#ffcb6b12}.event-list b{color:#ffcb6b;font:700 8px/1 ui-monospace,monospace}.delete:disabled{opacity:.35}
</style>
