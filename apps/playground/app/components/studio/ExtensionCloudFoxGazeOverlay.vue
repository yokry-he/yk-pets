<!--
  ...
  按共享 Shape Profile 对齐圆润/椭圆眼的动态追视高光；星芒、菱眼和目镜保留各自完整轮廓，不叠加圆形覆盖层。
  Aligns dynamic gaze highlights for round/oval eyes through the shared Shape Profile while leaving spark, diamond, and visor silhouettes unobscured.
-->
...
const props = withDefaults(defineProps<{
  ...
  fireworkSeed?: number
}>(), { pointer: () => ({ x: 0, y: 0 }), fireworkSeed: 0 })
...
const baseHighlightY = computed(() => scheme.model.head.eyeOffset[1] + profile.value.eyeY + scheme.model.head.eyeHighlightPosition[1])
const highlightZ = computed(() => scheme.model.head.eyeOffset[2] + profile.value.eyeZ + scheme.model.head.eyeHighlightPosition[2] + .012)
...
const supportsGazeOverlay = computed(() => ['round', 'oval'].includes(props.appearance.parts.eyes))
function setHighlight(node: unknown, side: number) { if (side < 0) leftHighlight.value = node as Group | undefined; else rightHighlight.value = node as Group | undefined }
...
useLoop().onBeforeRender(({ elapsed, delta }) => {
  ...
  const active = supportsGazeOverlay.value && (props.behavior === 'playing-ball' || props.behavior === 'diving-catch' || props.behavior === 'fireworks-show' || pointerActive)
  if (root.value) root.value.visible = active
  if (!active) return
  let gazeX = 0; let gazeY = 0
  if (props.behavior === 'playing-ball' || props.behavior === 'diving-catch') {
    ...
    gazeX = clamp(target.x * .125, -.095, .095); gazeY = clamp((target.y + .08) * .055, -.052, .058)
  }
    ...
    const burst = createProductionFireworkBurstPlan(props.fireworkSeed, burstIndex); gazeX = clamp(burst.originX * .025, -.045, .045); gazeY = .052
  }
  else { gazeX = clamp(props.pointer.x * .038, -.052, .052); gazeY = clamp(-props.pointer.y * .036, -.052, .052) }
  const blinkPhase = (elapsed + .73) % 4.8; const blinkDistance = Math.abs(blinkPhase - 4.64); const blinkScale = blinkDistance < .13 ? .12 + blinkDistance / .13 * .88 : 1
  ...
  update(leftHighlight.value, -1); update(rightHighlight.value, 1)
})
</script>

<template>
  <TresGroup ref="root" :visible="false" :position="headPosition" :rotation="headRotation" :scale="headScale">
    <template v-for="side in sides" :key="`gaze-${side}`">
      <TresMesh :position="vector(side * eyeX + side * baseHighlightX, baseHighlightY, highlightZ - .006)" :scale="vector(.058, .078, .031)"><TresSphereGeometry :args="[1, 18, 18]" /><TresMeshBasicMaterial :color="appearance.palette.eye" /></TresMesh>
      <TresGroup :ref="node => setHighlight(node, side)" :position="vector(side * eyeX + side * baseHighlightX, baseHighlightY, highlightZ)"><TresMesh :scale="vector(.048, .068, .026)"><TresSphereGeometry :args="[1, 20, 20]" /><TresMeshBasicMaterial :color="colors.eyeHighlight" :tone-mapped="false" /></TresMesh></TresGroup>
    </template>
  </TresGroup>
</template>
