...
<template>
  <section class="curve-editor">
    <header><div><small>曲线编辑</small><h3>曲线编辑器</h3></div><code>{{ channelId }} · {{ layerId }}</code></header>
    ...
    <div class="interpolations"><button v-for="item in ['step','linear','smooth','bezier'] as MotionInterpolation[]" :key="item" :disabled="!selectedKeyframeIds.length" @click="emit('interpolation',item)">{{ interpolationLabels[item] }}</button></div>
    <div class="tangents"><label>入切线<input v-model.number="inTangent" type="number" min="-8" max="8" step=".05"></label><label>出切线<input v-model.number="outTangent" type="number" min="-8" max="8" step=".05"></label><button :disabled="!selectedKeyframeIds.length" @click="emit('tangents',inTangent,outTangent)">应用切线</button></div>
  </section>
</template>
<style scoped>
.curve-editor{box-sizing:border-box;display:grid;gap:8px;min-width:0;max-width:100%;overflow:hidden;padding:9px;border:1px solid #ffffff12;border-radius:10px;background:#080d18}.curve-editor header{display:flex;justify-content:space-between;gap:8px;min-width:0}.curve-editor header>div{min-width:0}.curve-editor small{color:#75809e;font:800 8px/1 ui-monospace,monospace;letter-spacing:.08em}.curve-editor h3{margin:3px 0 0;font-size:12px}.curve-editor code{max-width:120px;overflow:hidden;color:#6fded2;font-size:7px;text-overflow:ellipsis;white-space:nowrap}svg{box-sizing:border-box;width:100%;height:128px;border:1px solid #ffffff12;border-radius:8px;background:#050812}.grid{fill:none;stroke:#ffffff0d;stroke-width:.5}.curve{fill:none;stroke:#6ff0df;stroke-width:1.4;vector-effect:non-scaling-stroke}circle{fill:#8792b2;stroke:#11182a;stroke-width:1;vector-effect:non-scaling-stroke}circle.selected{fill:#ff6f9d}.interpolations{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:4px}.tangents{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;align-items:end}.tangents button{grid-column:1/-1}.curve-editor label{display:grid;gap:3px;min-width:0;color:#aeb8d2;font-size:8px}.curve-editor input,.curve-editor button{box-sizing:border-box;width:100%;min-width:0;min-height:29px;padding:0 6px;border:1px solid #ffffff1d;border-radius:7px;color:#fff;background:#0b1020}.curve-editor button:disabled{opacity:.35}
</style>
